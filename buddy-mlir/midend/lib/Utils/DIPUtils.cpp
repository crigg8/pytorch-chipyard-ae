...
DIP_ERROR checkDIPCommonTypes(DIPOP op, const std::vector<Value> &args) {
  ...
  const auto getElementType = [&](int argIndex) {
    return mlir::cast<MemRefType>(getType(argIndex)).getElementType();
  };
...
Value getCenter(OpBuilder &builder, Location loc, MLIRContext *ctx, Value dim) {
  ...
  Value c1f = builder.create<arith::ConstantFloatOp>(
      loc, builder.getF32Type(), (llvm::APFloat)1.0f);
  Value c2f = builder.create<arith::ConstantFloatOp>(
...
std::vector<Value> extractIndices(OpBuilder &builder, Location loc, Value xVec,
  ...
  Value xPos = builder.create<vector::ExtractOp>(
      loc, xVec, SmallVector<OpFoldResult>{vecIndex});
  Value yPos = builder.create<vector::ExtractOp>(
      loc, yVec, SmallVector<OpFoldResult>{vecIndex});
      ...
      [&](OpBuilder &builder, Location loc, ValueRange ivs,
        ...
        Value xPos_temp =
            builder.create<vector::ExtractOp>(
                loc, xVecWeight, SmallVector<OpFoldResult>{ivs[0]});
        Value yPos_temp =
            builder.create<vector::ExtractOp>(
                loc, yVecWeight, SmallVector<OpFoldResult>{ivs[0]});
