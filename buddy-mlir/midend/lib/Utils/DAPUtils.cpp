...
SmallVector<Value, 5> generateSOSParams(OpBuilder &rewriter, Location loc,
  ...
  auto vecDistribute = rewriter.create<scf::ForOp>(
      ...
      [&](OpBuilder &builder, Location loc, Value iv, ValueRange iargs) {
        ...
        SmallVector<OpFoldResult> dynamicPosition{iv};
        Value b0Next = builder.create<vector::InsertOp>(
            loc, b0, iargs[0], dynamicPosition);
        Value b1Next = builder.create<vector::InsertOp>(
            loc, b1, iargs[1], dynamicPosition);
        Value b2Next = builder.create<vector::InsertOp>(
            loc, b2, iargs[2], dynamicPosition);
        Value a1Next = builder.create<vector::InsertOp>(
            loc, a1, iargs[3], dynamicPosition);
        Value a2Next = builder.create<vector::InsertOp>(
            loc, a2, iargs[4], dynamicPosition);
...
void biquadProcess(OpBuilder &rewriter, Location loc, VectorType vectorTy,
  ...
  auto injectionResult = rewriter.create<scf::ForOp>(
      ...
      [&](OpBuilder &builder, Location loc, Value iv, ValueRange iargs) {
        ...
        Value vecInNext = builder.create<vector::InsertOp>(
            loc, inElem, vecInMoveRight, SmallVector<OpFoldResult>{c0});
        Value vecOutNext =
  ...
  auto processResult = rewriter.create<scf::ForOp>(
      ...
      [&](OpBuilder &builder, Location loc, Value iv, ValueRange iargs) {
        ...
        Value vecOutNext =
            builder.create<vector::FMAOp>(loc, vecB0, vecInNext, iargs[1]);
        Value outElem = builder.create<vector::ExtractOp>(
            loc, vecOutNext, SmallVector<OpFoldResult>{cUpperBound});
  ...
  rewriter.create<scf::ForOp>(
      ...
      [&](OpBuilder &builder, Location loc, Value iv, ValueRange iargs) {
        Value vecInMoveRight = builder.create<vector::ShuffleOp>(
            loc, iargs[0], iargs[0], arrayRef);
        Value vecInNext = builder.create<vector::InsertOp>(
            loc, f0, vecInMoveRight, SmallVector<OpFoldResult>{c0});
        Value vecOutNext =
            builder.create<vector::FMAOp>(loc, vecB0, vecInNext, iargs[1]);
        Value outElem = builder.create<vector::ExtractOp>(
            loc, vecOutNext, SmallVector<OpFoldResult>{cUpperBound});
