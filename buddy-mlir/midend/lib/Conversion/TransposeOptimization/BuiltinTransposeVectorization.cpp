...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    if (permutationArrayAttr[0] != 1 or permutationArrayAttr[1] != 0 or
        mlir::cast<MemRefType>(A.getType()).getRank() != 2) {
      return failure();
    }
    ...
    affine::buildAffineLoopNest(
        ...
        [&](OpBuilder &builder, Location loc, ValueRange ivRange) {
          ...
          if (mlir::cast<MemRefType>(A.getType()).isDynamicDim(0) or
              mlir::cast<MemRefType>(A.getType()).getDimSize(0) % affineVectorSize !=
                  0) {
    ...
    if (mlir::cast<MemRefType>(A.getType()).isDynamicDim(1) or
        mlir::cast<MemRefType>(A.getType()).getDimSize(1) % affineVectorSize != 0) {
