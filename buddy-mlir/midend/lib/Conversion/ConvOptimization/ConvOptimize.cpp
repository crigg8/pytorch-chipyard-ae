...
public:
  ...
  LogicalResult matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/, ConversionPatternRewriter &rewriter) const override {
    ...
    ShapedType inputTy = mlir::cast<ShapedType>(input.getType());
                ...
                affine::buildAffineLoopNest(rewriter, loc, c0, g, kernelN * vecSize, [&](OpBuilder &builder, Location loc, ValueRange ivRange) {
                  ...
                  for (int i = 0; i < kernelM; ++i) {
                    ...
                    for (int j = 0; j < kernelN; ++j) {
                      ...
                      auto inputMap = AffineMap::getMinorIdentityMap(
                          /*numDims=*/4, /*numResults=*/1,
                          rewriter.getContext());
                      Value i = builder.create<TransferReadOp>(
                          loc, vecTy, input,
                          ValueRange{ivA, ivE, rowInput, columnInput},
                          std::nullopt, AffineMapAttr::get(inputMap),
