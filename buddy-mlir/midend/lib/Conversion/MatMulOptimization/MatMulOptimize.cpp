...
public:
  ...
  matchAndRewrite(Operation *op, ArrayRef<Value> /*operands*/,
    ...
    // ShapedType BTy = mlir::cast<ShapedType>(B.getType());
    // ShapedType CTy = mlir::cast<ShapedType>(C.getType());
    ...
    const AffineMap lastDimMap = AffineMap::get(2, 0, d1);
                    ...
                    [&](OpBuilder &builder, Location loc, ValueRange ivRange) {
                      ...
                      for (int i = 0; i < kernelM; ++i) {
                        Value a = builder.create<TransferReadOp>(
                            ...
                            std::nullopt, AffineMapAttr::get(mapBroadcast),
                      ...
                      for (int i = 0; i < kernelM; ++i) {
                        ...
                        for (int j = 0; j < kernelN; ++j) {
                          ...
                          Value d = builder.create<TransferReadOp>(
                              ...
                              std::nullopt, AffineMapAttr::get(lastDimMap),
                              rewriter.getBoolArrayAttr({true}));
                          ds.push_back(d);
                        }
                      }
                      for (int i = 0; i < kernelN; ++i) {
                        ...
                        Value b = builder.create<TransferReadOp>(
                            loc, vTy, B, ValueRange{ivK, fixedIV}, std::nullopt,
                            AffineMapAttr::get(lastDimMap),
                            rewriter.getBoolArrayAttr({true}));
                        bs.push_back(b);
                      }
                      ...
                      for (int i = 0; i < kernelM; ++i) {
                        for (int j = 0; j < kernelN; ++j) {
                          ...
                          builder.create<TransferWriteOp>(
                              ...
                              AffineMapAttr::get(lastDimMap),
                              Value(), rewriter.getBoolArrayAttr({true}));
                        }
                      }
                    });
              });
