...
int64_t getNumberFromValue(Value value) {
  ...
  auto valueAttr =
      defOp ? dyn_cast_or_null<IntegerAttr>(defOp->getAttr("value")) : nullptr;
  assert(valueAttr && "expected integer constant");
  return valueAttr.getInt();
}

bool isConstantIntValue(Value value, int64_t expected) {
  auto *defOp = value.getDefiningOp();
  auto valueAttr =
      defOp ? dyn_cast_or_null<IntegerAttr>(defOp->getAttr("value")) : nullptr;
  return valueAttr && valueAttr.getInt() == expected;
}

Value createI64Constant(Location loc, int64_t value,
                        ConversionPatternRewriter &rewriter) {
  return rewriter.create<LLVM::ConstantOp>(loc, rewriter.getI64Type(),
                                           rewriter.getI64IntegerAttr(value));
}

Value mulI64(Location loc, Value lhs, Value rhs,
             ConversionPatternRewriter &rewriter) {
  return rewriter.create<LLVM::MulOp>(loc, lhs.getType(), lhs, rhs);
}

Value addI64(Location loc, Value lhs, Value rhs,
             ConversionPatternRewriter &rewriter) {
  return rewriter.create<LLVM::AddOp>(loc, lhs.getType(), lhs, rhs);
}

Value mulI64ByConstant(Location loc, Value value, int64_t multiplier,
                       ConversionPatternRewriter &rewriter) {
  if (multiplier == 1)
    return value;
  return mulI64(loc, value, createI64Constant(loc, multiplier, rewriter),
                rewriter);
}

Value createMatrixByteOffset(Location loc, Value rowStride, size_t row,
                             size_t col, size_t elementSize,
                             ConversionPatternRewriter &rewriter) {
  Value rowValue = createI64Constant(loc, row, rewriter);
  Value colValue = createI64Constant(loc, col, rewriter);
  Value linearOffset = addI64(loc, mulI64(loc, rowValue, rowStride, rewriter),
                              colValue, rewriter);
  return mulI64ByConstant(loc, linearOffset, elementSize, rewriter);
}

Value createMatrixTileAddress(Location loc, Value base, Value rowStride,
                              size_t row, size_t col, size_t elementSize,
                              ConversionPatternRewriter &rewriter) {
  Value byteOffset =
      createMatrixByteOffset(loc, rowStride, row, col, elementSize, rewriter);
  return addI64(loc, base, byteOffset, rewriter);
}

struct MatrixDescriptorInfo {
  Value address;
  Value outerStride;
  Value innerStride;
};

MatrixDescriptorInfo extractMatrixDescriptorInfo(Value descriptor,
                                                 size_t elementSize,
                                                 Location loc,
                                                 ConversionPatternRewriter &rewriter) {
  ...
  Value basePtr =
      rewriter.create<LLVM::PtrToIntOp>(loc, rewriter.getI64Type(), alignedPtr);
  Value offset = memrefDescriptor.offset(rewriter, loc);
  Value outerStride = memrefDescriptor.stride(rewriter, loc, 0);
  Value innerStride = memrefDescriptor.stride(rewriter, loc, 1);
  Value byteOffset = mulI64ByConstant(loc, offset, elementSize, rewriter);
  return {addI64(loc, basePtr, byteOffset, rewriter), outerStride, innerStride};
}
...
void gemminiMvinOffset(const Value &mem, const Value &offset,
  ...
  Value configPtr = rewriter.create<LLVM::AddOp>(loc, i64Type, mem, offset);
  uint64_t spadAddrInt = (uint64_t)rows << (addrLen + 16) |
                         (uint64_t)cols << addrLen | (uint64_t)SpAddr;
  Value spad = createI64Constant(loc, spadAddrInt, rewriter);
...
void gemminiMvinOffset(const Value &mem, const size_t offset,
                       ...
                       ConversionPatternRewriter &rewriter) {
  Location loc = mem.getLoc();
  Value offsetOp = createI64Constant(loc, offset, rewriter);
  gemminiMvinOffset<IntrOp>(mem, offsetOp, SpAddr, cols, rows, addrLen,
                            rewriter);
}

void gemminiMvoutOffset(const Value &mem, const Value &offset,
                        const uint32_t SpAddr, const size_t cols,
                        const size_t rows, int64_t addrLen,
                        ConversionPatternRewriter &rewriter) {
  Location loc = mem.getLoc();
...
void gemminiMvoutOffset(const Value &mem, const size_t offset,
  ...
  Location loc = mem.getLoc();
  Value offsetOp = createI64Constant(loc, offset, rewriter);
  gemminiMvoutOffset(mem, offsetOp, SpAddr, cols, rows, addrLen, rewriter);
}
...
struct GemminiConfigStLowering : public ConvertOpToLLVMPattern<ConfigStOp> {
  ...
  matchAndRewrite(ConfigStOp configStOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Value strideValue = adaptor.getStride();
    ...
    Value value1 = createI64Constant(loc, rs1, rewriter);
    Value highBits = createI64Constant(
        loc,
        (uint64_t)acc_scale_t_to_acc_scale_t_bits((acc_scale_t)scale) << 32,
        rewriter);
    Value lowMask = createI64Constant(loc, 0xffffffffULL, rewriter);
    Value maskedStride = rewriter.create<LLVM::AndOp>(
        loc, strideValue.getType(), strideValue, lowMask);
    Value value2 = rewriter.create<LLVM::OrOp>(loc, highBits.getType(),
                                               highBits, maskedStride);
...
struct GemminiConfigLdLowering : public ConvertOpToLLVMPattern<ConfigLdOp> {
  ...
  matchAndRewrite(ConfigLdOp configLdOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Value rs2Value = adaptor.getStride();
...
class GemminiTileMatMulLowering : public ConvertOpToLLVMPattern<TileMatMulOp> {
  void gemminiLoopWs(size_t i, size_t j, size_t k, size_t padI, size_t padJ,
                     ...
                     Value aRowStride, Value bRowStride, Value dRowStride,
                     Value cRowStride, bool aTranspose, bool bTranspose,
    ...
    rewriter.create<LoopWsConfigStridesAB_IntrOp>(loc, aRowStride, bRowStride);
    // loopWsConfigStrideDC instruction
    rewriter.create<LoopWsConfigStridesDC_IntrOp>(loc, dRowStride, cRowStride);
  ...
  void spTiledMatmulWs(Value &a, Value &b, Value &d, Value &c,
                       ...
                       size_t padI, size_t padJ, size_t padK, Value strideA,
                       Value strideB, Value strideD, Value strideC,
                       bool aTranspose, bool bTranspose, bool fullC, bool lowD,
                       bool noBias, bool repeatingBias, int act,
                       TileMatMulOp &tileMatMulOp,
                       ConversionPatternRewriter &rewriter) const {
    Location loc = a.getLoc();
    Value dStride = repeatingBias ? createI64Constant(loc, 0, rewriter) : strideD;
    gemminiLoopWs(i, j, k, padI, padJ, padK, a, b, d, c, strideA, strideB,
                  dStride, strideC, aTranspose, bTranspose, fullC, lowD,
                  !noBias, act, tileMatMulOp, rewriter);
  }
  ...
  void spTiledMatmulOs(Value &a, Value &b, Value &d, Value &c,
    ...
    const size_t maxBlockLen = MAX_BYTES / (dim * sizeOfElemT);
    const size_t maxBlockLenAcc = MAX_BYTES / (dim * sizeOfAccT);

    const int aBlocks = k <= maxBlockLen ? k : maxBlockLen;
    const int bBlocks = j <= maxBlockLen ? j : maxBlockLen;
    const int dBlocks = (j <= maxBlockLenAcc ? j : maxBlockLenAcc) > 0 ? 
                        (j <= maxBlockLenAcc ? j : maxBlockLenAcc) : 1;

    Location loc = a.getLoc();
    bool dAddrNull = isConstantIntValue(d, 0);
    bool cAddrNull = isConstantIntValue(c, 0);

    // Move-in D
    if (!dAddrNull && !noBias) {
      Value strideValue = repeatingBias
                              ? createI64Constant(loc, 0, rewriter)
                              : mulI64ByConstant(loc, strideD, sizeOfAccT,
                                                 rewriter);
      ...
      for (size_t i0 = 0; i0 < i; i0++) {
        for (size_t j0 = 0; j0 < j; j0 += dBlocks) {
          ...
          Value offset = createMatrixByteOffset(
              loc, strideD, biasRow * dim, j0 * dim, sizeOfAccT, rewriter);
    ...
    for (size_t i0 = 0; i0 < i; i0++) {
      for (size_t k0 = 0; k0 < k; k0 += aBlocks) {
        Value offset = createMatrixByteOffset(
            loc, strideA, i0 * dim, k0 * dim, sizeOfElemT, rewriter);
    ...
    if (!cAddrNull) {
      ...
      for (size_t i0 = 0; i0 < i; i0++) {
        for (size_t j0 = 0; j0 < j; j0++) {
          Value offset = createMatrixByteOffset(
              loc, strideC, i0 * dim, j0 * dim, sizeof_C, rewriter);
          const uint32_t cSpAddr = cSpAddrStart + (i0 * j + j0) * dim;
  ...
  void tiledMatmulOuter(
      ...
      Value &C, Value strideA, Value strideB, Value strideD, Value strideC,
      ...
      bool aTranspose, bool bTranspose, bool fullC, bool lowD, int dataflow,
    ...
    Value strideValue = mulI64ByConstant(loc, strideC, sizeofC, rewriter);
    rewriter.create<ConfigStOp>(loc, strideValue, act & 3,
                                llvm::APFloat(scale));
    strideValue = mulI64ByConstant(loc, strideA, sizeOfElemT, rewriter);
    rewriter.create<ConfigLdOp>(loc, strideValue, llvm::APFloat(aScaleFactor),
                                false, 0);
    strideValue = mulI64ByConstant(loc, strideB, sizeOfElemT, rewriter);
    rewriter.create<ConfigLdOp>(loc, strideValue, llvm::APFloat(bScaleFactor),
                                false, 1);
    strideValue = mulI64ByConstant(loc, strideD, sizeofD, rewriter);
    rewriter.create<ConfigLdOp>(loc, strideValue,
    ...
    for (size_t i0 = 0; i0 < I0; i0++){
      for (size_t j0 = 0; j0 < J0; j0++){
        for (size_t k0 = 0; k0 < K0; k0++) {
          ...
          Location loc = A.getLoc(); 
          if (k0 != 0) { 
            pre = createI64Constant(loc, 0, rewriter); 
          } else { 
            size_t biasRow = repeatingBias ? 0 : i0 * tileI * dim; 
            pre = createMatrixTileAddress(loc, D, strideD, biasRow, 
                                          j0 * tileJ * dim, sizeofD, rewriter); 
          } 
 
          Value out; 
          if (k0 == K0 - 1) { 
            out = createMatrixTileAddress(loc, C, strideC, i0 * tileI * dim, 
                                          j0 * tileJ * dim, sizeofC, rewriter); 
          } else { 
            out = createI64Constant(loc, 0, rewriter); 
          } 
          ...
          Value a; 
          if (aTranspose) { 
            a = createMatrixTileAddress(loc, A, strideA, k0 * tileK * dim, 
                                        i0 * tileI * dim, sizeOfElemT, rewriter); 
          } else { 
            a = createMatrixTileAddress(loc, A, strideA, i0 * tileI * dim, 
                                        k0 * tileK * dim, sizeOfElemT, rewriter); 
          } 
          Value b; 
          if (bTranspose) { 
            b = createMatrixTileAddress(loc, B, strideB, j0 * tileJ * dim, 
                                        k0 * tileK * dim, sizeOfElemT, rewriter); 
          } else { 
            b = createMatrixTileAddress(loc, B, strideB, k0 * tileK * dim, 
                                        j0 * tileJ * dim, sizeOfElemT, rewriter); 
          } 
...
public:
  ...
  matchAndRewrite(TileMatMulOp tileMatMulOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto modOp = tileMatMulOp->getParentOfType<mlir::ModuleOp>();
    size_t dbStages = 2;

    size_t dbPartitionRows = ((BANK_NUM * bankRows / dbStages) / 2);
    size_t dbMatsInPartition = (dbPartitionRows / dim);
    size_t dbMatsInAcc((accRows / dbStages) / dim);
    ...
    Value aArrayDesc = adaptor.getAArray();
    Value bArrayDesc = adaptor.getBArray();
    Value cArrayDesc = adaptor.getCArray();
    Value dArrayDesc = adaptor.getDArray();
    ...
    MatrixDescriptorInfo aArrayInfo =
        extractMatrixDescriptorInfo(aArrayDesc, sizeOfElemT, loc, rewriter);
    MatrixDescriptorInfo bArrayInfo =
        extractMatrixDescriptorInfo(bArrayDesc, sizeOfElemT, loc, rewriter);
    MatrixDescriptorInfo cArrayInfo = extractMatrixDescriptorInfo(
        cArrayDesc, fullC ? sizeOfAccT : sizeOfElemT, loc, rewriter);
    MatrixDescriptorInfo dArrayInfo = extractMatrixDescriptorInfo(
        dArrayDesc, lowD ? sizeOfElemT : sizeOfAccT, loc, rewriter);
    ...
    Value aArrayindexCastOp = aArrayInfo.address;
    Value bArrayindexCastOp = bArrayInfo.address;
    Value cArrayindexCastOp = cArrayInfo.address;
    Value dArrayindexCastOp = dArrayInfo.address;
    Value strideA = aArrayInfo.outerStride;
    Value strideB = bArrayInfo.outerStride;
    Value strideC = cArrayInfo.outerStride;
    Value strideD = dArrayInfo.outerStride;
    if (bTranspose)
      strideB = bArrayInfo.innerStride;
    ...
    auto readTileAttr = [&](llvm::StringRef name, size_t &tile) {
      auto attr = modOp->getAttrOfType<IntegerAttr>(name);
      if (!attr || attr.getInt() < 0)
        return false;
      tile = static_cast<size_t>(attr.getInt());
      return true;
    };
    size_t requestedTileI = 0;
    size_t requestedTileJ = 0;
    size_t requestedTileK = 0;
    bool hasRequestedTileI =
        readTileAttr("ttc.gemmini_tile_i", requestedTileI);
    bool hasRequestedTileJ =
        readTileAttr("ttc.gemmini_tile_j", requestedTileJ);
    bool hasRequestedTileK =
        readTileAttr("ttc.gemmini_tile_k", requestedTileK);
    bool useRequestedTile =
        hasRequestedTileI && hasRequestedTileJ && hasRequestedTileK &&
        requestedTileI > 0 && requestedTileJ > 0 && requestedTileK > 0 &&
        requestedTileI * dim <= dimIPaded &&
        requestedTileJ * dim <= dimJPaded &&
        requestedTileK * dim <= dimKPaded &&
        tiledMatmulTotalSpadRows(requestedTileI, requestedTileJ,
                                 requestedTileK) <= maxSpadRows &&
        tiledMatmulTotalAccRows(requestedTileI, requestedTileJ) <= maxAccRows;
    if (useRequestedTile) {
      tileI = requestedTileI;
      tileJ = requestedTileJ;
      tileK = requestedTileK;
    }
    ...
    tiledMatmulOuter(dimI, dimJ, dimK, aArrayindexCastOp, bArrayindexCastOp,
                     ...
                     tileI, tileJ, tileK, act, scale, bertScale,
                     repeatingBias, aTranspose, bTranspose, fullC, lowD,
                     dataflow, tileMatMulOp, rewriter);
    return success();
  };
