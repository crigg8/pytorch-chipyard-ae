...
PtrState::createSideBySideCastOps(ArrayRef<int64_t> resultShape,
  ...
  // Side-by-side wrap is bounded by the logical column extent, but
  // targetOffset is already expressed in linearized element units.
  Value modN = rewriter.create<arith::IndexCastOp>(loc, rewriter.getIndexType(),
                                                   modulos[1]->size);
  Value strideCol = strideVals[1];
  Value modNLinear = rewriter.create<arith::MulIOp>(loc, modN, strideCol);
...
void PtrAnalysis::visitOperandRem(
  ...
  if (state.getRank() == 1) {
    ...
    if (shape[0] == 1) {
      Value modulo = rhsState.scalar;
      if (auto strideAttr = getIntAttr(state.strides[1])) {
        if (strideAttr.value() > 1) {
          Value stride = ofrToIndexValue(state.strides[1], loc, rewriter);
          modulo = rewriter.create<arith::DivSIOp>(loc, modulo, stride);
        }
      } else {
        Value stride = ofrToIndexValue(state.strides[1], loc, rewriter);
        modulo = rewriter.create<arith::DivSIOp>(loc, modulo, stride);
      }
      state.modulos[1] = ModuloState{modulo};
    } else if (shape[1] == 1) {
      Value modulo = rhsState.scalar;
      if (auto strideAttr = getIntAttr(state.strides[0])) {
        if (strideAttr.value() > 1) {
          Value stride = ofrToIndexValue(state.strides[0], loc, rewriter);
          modulo = rewriter.create<arith::DivSIOp>(loc, modulo, stride);
        }
      } else {
        Value stride = ofrToIndexValue(state.strides[0], loc, rewriter);
      ...
      state.modulos[0] = ModuloState{modulo};
    } else {
