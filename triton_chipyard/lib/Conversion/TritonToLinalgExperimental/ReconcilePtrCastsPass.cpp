...
struct ReconcileMemrefLayoutCasts
    : public OpRewritePattern<UnrealizedConversionCastOp> {
  ReconcileMemrefLayoutCasts(MLIRContext *context, PatternBenefit benefit = 1)
      : OpRewritePattern<UnrealizedConversionCastOp>(context, benefit) {}
  ...
  LogicalResult matchAndRewrite(UnrealizedConversionCastOp op,
    ...
    // Only rewrite pure layout-only casts. Shape/element/memory-space changing
    // casts are intentionally left untouched.
    if (inputType.getShape() != outputType.getShape() ||
        inputType.getElementType() != outputType.getElementType() ||
        inputType.getMemorySpace() != outputType.getMemorySpace())
      return failure();

    int64_t inOffset = ShapedType::kDynamic;
    int64_t outOffset = ShapedType::kDynamic;
    SmallVector<int64_t> inStrides;
    SmallVector<int64_t> outStrides;
    if (failed(inputType.getStridesAndOffset(inStrides, inOffset)) ||
        failed(outputType.getStridesAndOffset(outStrides, outOffset)))
      return failure();
    if (inOffset == outOffset && inStrides == outStrides)
      return failure();

    // Keep this patch local: only handle casts that come directly from
    // reinterpret_cast, i.e., where layout metadata is explicitly materialized.
    if (!input.getDefiningOp<memref::ReinterpretCastOp>())
      return failure();
    ...
    for (Operation *user : output.getUsers()) {
      ...
      if (!subview || subview.getSource() != output)
        return failure();

      // Keep the transformation constrained to the exact copy-in staging pattern.
      for (Operation *subviewUser : subview->getUsers()) {
        auto copyOp = dyn_cast<memref::CopyOp>(subviewUser);
        if (!copyOp || copyOp.getSource() != subview.getResult())
          return failure();
      }
      subviewUsers.push_back(subview);
    }

    // Rebuild each subview from the original strided source so inferred result
    // types keep the source layout information.
    for (memref::SubViewOp subview : subviewUsers) {
      OpBuilder::InsertionGuard guard(rewriter);
      rewriter.setInsertionPoint(subview);
      auto rewrittenSubview = rewriter.create<memref::SubViewOp>(
          subview.getLoc(), input, subview.getMixedOffsets(),
          subview.getMixedSizes(), subview.getMixedStrides());
      rewriter.replaceOp(subview, rewrittenSubview.getResult());
    }
  ...
  void runOnOperation() override {
    ...
    patterns
        .add<SimplifyUnrealizedCast, FromMemrefConverter, ToMemrefConverter,
             ReconcileMemrefLayoutCasts>(&getContext());
