...
static Type getPtrPointeeElementType(Type ptrLikeType) {
  Type ptrTypeLike = ptrLikeType;
  if (auto shapedType = dyn_cast<ShapedType>(ptrTypeLike)) {
    ptrTypeLike = shapedType.getElementType();
  }

  auto ptrType = dyn_cast<triton::PointerType>(ptrTypeLike);
  if (!ptrType) {
    return nullptr;
  }

  Type pointeeType = ptrType.getPointeeType();
  if (auto shapedPointeeType = dyn_cast<ShapedType>(pointeeType)) {
    return shapedPointeeType.getElementType();
  }
  return pointeeType;
}
...
static bool isStorageCompatiblePtrLike(Type srcPtrType, Type dstPtrLikeType) {
  auto srcElemType = getPtrPointeeElementType(srcPtrType);
  auto dstElemType = getPtrPointeeElementType(dstPtrLikeType);
...
static Value materializeTypedBasePtr(Value source, Type ptrLikeType,
  ...
  Type dstElemType = getPtrPointeeElementType(ptrLikeType);
  if (!dstElemType) {
    return source;
  }
  ...
  if (!isStorageCompatiblePtrLike(source.getType(), ptrLikeType)) {
    return source;
  }

  return builder.create<triton::BitcastOp>(loc, dstPtrType, source).getResult();
}

static bool hasOnlyStructuredTensorPtrUsers(Value value) {
  return !value.use_empty() &&
         llvm::all_of(value.getUsers(), [](Operation *user) {
           return isa<tts::MakeTensorPtrOp, tts::MakeGatherScatterTensorPtrOp>(
               user);
         });
}
        ...
        auto res =
            llvm::TypeSwitch<Operation *, LogicalResult>(user)
                ...
                .Case<triton::BitcastOp>([&](triton::BitcastOp op) {
                  ...
                  // Preserve typed scalar base pointers that were introduced
                  // for structured tensor pointer descriptors. Those bitcasts
                  // must remain visible to the structured lowering path.
                  if (!isa<ShapedType>(resType) &&
                      hasOnlyStructuredTensorPtrUsers(res)) {
                    return success();
                  }

                  auto ptr = op.getOperand();
                  auto offsetInfo = offsetMap.at(ptr);
      ...
      auto res =
          llvm::TypeSwitch<Operation *, LogicalResult>(op)
              .Case<triton::LoadOp>([&](triton::LoadOp load) {
                ...
                auto typedPtr = materializeTypedBasePtr(
                    offsetInfo.ptr, offsetInfo.ptrType, loc, b);
                auto gather = b.create<tts::GatherOp>(
                    loc, load.getType(), typedPtr, offsetInfo.offset,
                    load.getMask(), other);
              ...
              .Case<triton::StoreOp>([&](triton::StoreOp store) {
                ...
                auto typedPtr = materializeTypedBasePtr(
                    offsetInfo.ptr, offsetInfo.ptrType, loc, b);
                b.create<tts::ScatterOp>(loc, typedPtr, offsetInfo.offset,
              ...
              .Case<triton::MakeTensorPtrOp,
                ...
                makeTensorPtr.getBaseMutable().set(typedPtr);
