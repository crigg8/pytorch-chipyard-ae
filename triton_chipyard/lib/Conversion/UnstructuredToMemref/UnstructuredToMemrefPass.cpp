  ...
  PtrToUnrankedMemrefConverter() {
    ...
    addConversion([](triton::PointerType ptrType) {
      ...
      if (elemType.isInteger(1)) {
        elemType = IntegerType::get(ptrType.getContext(), 8);
      }
      return UnrankedMemRefType::get(elemType, 0);
    });
...
static Type getStorageElementType(Type elemType, MLIRContext *context) {
  if (elemType.isInteger(1)) {
...
static Value convertValueForStorage(OpBuilder &builder, Location loc,
                                    Value value, Type storageElemType) {
  auto valueType = value.getType();
  if (valueType == storageElemType) {
    return value;
  }

  if (valueType.isInteger(1) && storageElemType.isInteger(8)) {
    return builder.create<arith::ExtUIOp>(loc, storageElemType, value);
  }

  if (valueType.isInteger(8) && storageElemType.isInteger(1)) {
    return builder.create<arith::TruncIOp>(loc, storageElemType, value);
  }

  return value;
}

static Value convertValueFromStorage(OpBuilder &builder, Location loc,
                                     Value value, Type logicalElemType) {
  auto valueType = value.getType();
  if (valueType == logicalElemType) {
    return value;
  }

  if (valueType.isInteger(8) && logicalElemType.isInteger(1)) {
    return builder.create<arith::TruncIOp>(loc, logicalElemType, value);
  }

  if (valueType.isInteger(1) && logicalElemType.isInteger(8)) {
    return builder.create<arith::ExtUIOp>(loc, logicalElemType, value);
  }
...
static MemRefType getMemrefTypeForScalarPtr(triton::PointerType ptrType,
  ...
  auto elemType = getStorageElementType(ptrType.getPointeeType(), context);
...
struct ScalarLoadConverter : public OpConversionPattern<tts::GatherOp> {
  ...
  matchAndRewrite(tts::GatherOp gatherOp, OpAdaptor adaptor,
    ...
    auto loadValue = convertValueFromStorage(
        rewriter, loc, scalarLoadOp.getResult(), gatherOp.getType());
    rewriter.replaceOp(gatherOp, loadValue);
...
struct ScalarStoreConverter : public OpConversionPattern<tts::ScatterOp> {
  ...
  matchAndRewrite(tts::ScatterOp scatterOp, OpAdaptor adaptor,
    ...
    auto storageElemType = getStorageElementType(
        cast<triton::PointerType>(scatterOp.getPtr().getType()).getPointeeType(),
        rewriter.getContext());
    auto storeVal =
        convertValueForStorage(rewriter, loc, scatterOp.getValue(),
                               storageElemType);
    auto zeroMap = AffineMap::getConstantMap(0, rewriter.getContext());
...
struct GatherConverter : public OpConversionPattern<tts::GatherOp> {
  ...
  matchAndRewrite(tts::GatherOp gatherOp, OpAdaptor adaptor,
    ...
    auto storageElemType =
        getStorageElementType(resultType.getElementType(), rewriter.getContext());
    ...
    auto genericOp = rewriter.create<linalg::GenericOp>(
        ...
        iteratorTypes, [&](OpBuilder &b, Location loc, ValueRange args) {
          auto getValueAtIndex = [baseTensor, resultType](OpBuilder &b, Location loc,
                                              Value index) -> Value {
            Value index0 =
                b.create<arith::IndexCastOp>(loc, b.getIndexType(), index);

            auto loaded = b.create<tensor::ExtractOp>(loc, baseTensor,
                                                      ValueRange{index0});
            return convertValueFromStorage(b, loc, loaded,
                                           resultType.getElementType());
          };
  ...
  matchAndRewrite(tts::ScatterOp scatterOp, OpAdaptor adaptor,
    ...
    auto genericOp = rewriter.create<linalg::GenericOp>(
        ...
        [&](OpBuilder &b, Location loc, ValueRange args) {
          ...
          auto offset = args[0];
          auto value = convertValueForStorage(b, loc, args[1], storageElemType);
