...
struct AssertConverter : public OpConversionPattern<triton::AssertOp> {
  ...
  matchAndRewrite(triton::AssertOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    // Original lowering (kept for reference):
    ...
    // Drop runtime asserts for backends/pipelines where cf.assert is unsupported.
    rewriter.eraseOp(op);
    return success();
  }
};

struct DropAssumeConverter : public OpRewritePattern<LLVM::AssumeOp> {
  using OpRewritePattern<LLVM::AssumeOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(LLVM::AssumeOp op,
                                PatternRewriter &rewriter) const override {
    // Drop llvm.assume before bufferization/deallocation. The downstream
    // buddy-mlir deallocation pipeline rejects ops without memory effects.
    rewriter.eraseOp(op);
    return success();
  }
};

struct DropGpuBarrierConverter : public OpConversionPattern<gpu::BarrierOp> {
...
// Match and lower the multi-result reduction emitted by
// torch._inductor.runtime.triton_helpers.welford().
struct WelfordConverter : public OpConversionPattern<triton::ReduceOp> {
  using OpConversionPattern<triton::ReduceOp>::OpConversionPattern;

private:
  static bool matchesBinaryInputs(Operation *op, Value lhs, Value rhs) {
    if (auto addOp = dyn_cast<arith::AddFOp>(op)) {
      return (addOp.getLhs() == lhs && addOp.getRhs() == rhs) ||
             (addOp.getLhs() == rhs && addOp.getRhs() == lhs);
    }
    if (auto mulOp = dyn_cast<arith::MulFOp>(op)) {
      return (mulOp.getLhs() == lhs && mulOp.getRhs() == rhs) ||
             (mulOp.getLhs() == rhs && mulOp.getRhs() == lhs);
    }
    return false;
  }

  static Value getInitTensor(ConversionPatternRewriter &rewriter,
                             ArrayRef<int64_t> shape, Type elementType,
                             Location loc) {
    if (!isa<FloatType>(elementType)) {
      return {};
    }
    Value initTensor = rewriter.create<tensor::EmptyOp>(loc, shape, elementType);
    Value zero = rewriter.create<arith::ConstantOp>(
        loc, elementType, rewriter.getFloatAttr(elementType, 0.0));
    return rewriter
        .create<linalg::FillOp>(loc, ValueRange{zero}, ValueRange{initTensor})
        .result();
  }
...
public:
  ...
  matchAndRewrite(ReduceOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    if (op.getBody()->getNumArguments() != 6 || op->getNumOperands() != 3 ||
        op->getNumResults() != 3) {
      return failure();
    }
    ...
    Value currMean = block->getArgument(0);
    Value currM2 = block->getArgument(1);
    Value currWeight = block->getArgument(2);
    Value redMean = block->getArgument(3);
    Value redM2 = block->getArgument(4);
    Value redWeight = block->getArgument(5);

    auto delta = dyn_cast_or_null<arith::SubFOp>(it != end ? &*it++ : nullptr);
    if (!delta || delta.getLhs() != redMean || delta.getRhs() != currMean) {
      return failure();
    }

    auto newWeight =
        dyn_cast_or_null<arith::AddFOp>(it != end ? &*it++ : nullptr);
    if (!newWeight ||
        !matchesBinaryInputs(newWeight.getOperation(), currWeight, redWeight)) {
      return failure();
    }

    auto cmpZero =
        dyn_cast_or_null<arith::CmpFOp>(it != end ? &*it++ : nullptr);
    if (!cmpZero || cmpZero.getPredicate() != arith::CmpFPredicate::OEQ) {
      return failure();
    }

    Value zeroConst;
    if (cmpZero.getLhs() == newWeight) {
      zeroConst = cmpZero.getRhs();
    } else if (cmpZero.getRhs() == newWeight) {
      zeroConst = cmpZero.getLhs();
    } else {
      return failure();
    }

    auto weightDiv =
        dyn_cast_or_null<arith::DivFOp>(it != end ? &*it++ : nullptr);
    if (!weightDiv || weightDiv.getLhs() != redWeight ||
        weightDiv.getRhs() != newWeight) {
      return failure();
    }

    auto weightRatio =
        dyn_cast_or_null<arith::SelectOp>(it != end ? &*it++ : nullptr);
    if (!weightRatio || weightRatio.getCondition() != cmpZero ||
        weightRatio.getTrueValue() != zeroConst ||
        weightRatio.getFalseValue() != weightDiv) {
      return failure();
    }

    auto scaledDelta =
        dyn_cast_or_null<arith::MulFOp>(it != end ? &*it++ : nullptr);
    if (!scaledDelta ||
        !matchesBinaryInputs(scaledDelta.getOperation(), delta, weightRatio)) {
      return failure();
    }
    ...
    auto outMean =
        dyn_cast_or_null<arith::AddFOp>(it != end ? &*it++ : nullptr);
    if (!outMean ||
        !matchesBinaryInputs(outMean.getOperation(), currMean, scaledDelta)) {
      return failure();
    }
    ...
    auto partialM2 =
        dyn_cast_or_null<arith::AddFOp>(it != end ? &*it++ : nullptr);
    if (!partialM2 ||
        !matchesBinaryInputs(partialM2.getOperation(), currM2, redM2)) {
      return failure();
    }
    ...
    auto deltaSquared =
        dyn_cast_or_null<arith::MulFOp>(it != end ? &*it++ : nullptr);
    if (!deltaSquared ||
        !matchesBinaryInputs(deltaSquared.getOperation(), delta, delta)) {
      return failure();
    }

    auto weightedDeltaSquared =
        dyn_cast_or_null<arith::MulFOp>(it != end ? &*it++ : nullptr);
    if (!weightedDeltaSquared ||
        !matchesBinaryInputs(weightedDeltaSquared.getOperation(), deltaSquared,
                             currWeight)) {
      return failure();
    }

    auto scaledWeightedDeltaSquared =
        dyn_cast_or_null<arith::MulFOp>(it != end ? &*it++ : nullptr);
    if (!scaledWeightedDeltaSquared ||
        !matchesBinaryInputs(scaledWeightedDeltaSquared.getOperation(),
                             weightedDeltaSquared,
                             weightRatio)) {
      return failure();
    }

    auto outM2 = dyn_cast_or_null<arith::AddFOp>(it != end ? &*it++ : nullptr);
    if (!outM2 ||
        !matchesBinaryInputs(outM2.getOperation(), partialM2,
                             scaledWeightedDeltaSquared)) {
      return failure();
    }
    ...
    if (termOp.getOperands() != ArrayRef<Value>{outMean, outM2, newWeight}) {
      return failure();
    }

    auto loc = op.getLoc();
    auto result0Type = dyn_cast<RankedTensorType>(op.getType(0));
    const bool isScalarReduce = result0Type == nullptr;
    SmallVector<int64_t> reductionResultShape{
        isScalarReduce ? SmallVector<int64_t>{}
                       : SmallVector<int64_t>(result0Type.getShape())};

    auto elemTypes = op.getElementTypes();
    if (elemTypes.size() != 3 ||
        !isa<FloatType>(elemTypes[0]) ||
        !isa<FloatType>(elemTypes[1]) ||
        !isa<FloatType>(elemTypes[2])) {
      return failure();
    }

    SmallVector<Value> outputs{
        getInitTensor(rewriter, reductionResultShape, elemTypes[0], loc),
        getInitTensor(rewriter, reductionResultShape, elemTypes[1], loc),
        getInitTensor(rewriter, reductionResultShape, elemTypes[2], loc)};
    if (llvm::any_of(outputs, [](Value v) { return !v; })) {
      return failure();
    }
    ...
    auto linalgOp = rewriter.create<linalg::ReduceOp>(
        ...
        [&](OpBuilder &b, Location nestedLoc, ValueRange inputs) {
          ...
          for (auto &nestedOp : tritonReduceBlock->without_terminator()) {
          ...
          b.create<linalg::YieldOp>(nestedLoc, results);
        });

    if (isScalarReduce) {
      SmallVector<Value> reduceResults;
      reduceResults.reserve(3);
      for (unsigned idx = 0; idx < elemTypes.size(); ++idx) {
        Type ty = elemTypes[idx];
        reduceResults.push_back(
            rewriter.create<tensor::ExtractOp>(loc, ty, linalgOp.getResults()[idx],
                                               ValueRange{}));
      }
...
private:
  static bool hasSingleUseBy(Value value, Operation *user) {
    return value.hasOneUse() && value.use_begin()->getOwner() == user;
  }

  static bool matchesPlainSelect(CmpOp cmpOp, arith::SelectOp selectOp) {
    return cmpOp.getResult() == selectOp.getCondition() &&
           cmpOp.getLhs() == selectOp.getTrueValue() &&
           cmpOp.getRhs() == selectOp.getFalseValue();
  }

  static LogicalResult
  matchSimpleMinMax(CmpOp cmpOp, arith::SelectOp &selectOp) {
    if (!cmpOp.getResult().hasOneUse()) {
      return failure();
    }
    selectOp = dyn_cast<arith::SelectOp>(*cmpOp.getResult().getUsers().begin());
    if (!selectOp || !matchesPlainSelect(cmpOp, selectOp)) {
      return failure();
    }
    return success();
  }
  ...
  static LogicalResult
  matchNaNAwareMinMax(CmpOp cmpOp, arith::CmpFOp &nanCmp, arith::OrIOp &orOp,
    ...
    if constexpr (!std::is_same_v<CmpOp, arith::CmpFOp>) {
      ...
      orOp = dyn_cast<arith::OrIOp>(*cmpOp.getResult().getUsers().begin());
      if (!orOp || !orOp.getResult().hasOneUse()) {
        return failure();
      }

      selectOp = dyn_cast<arith::SelectOp>(*orOp.getResult().getUsers().begin());
      if (!selectOp || selectOp.getCondition() != orOp ||
          cmpOp.getLhs() != selectOp.getTrueValue() ||
          cmpOp.getRhs() != selectOp.getFalseValue()) {
        return failure();
      }
      ...
      Value otherOperand =
          orOp.getLhs() == cmpOp
              ? orOp.getRhs()
              : (orOp.getRhs() == cmpOp ? orOp.getLhs() : Value{});
      if (!otherOperand) {
        return failure();
      }

      nanCmp = otherOperand.getDefiningOp<arith::CmpFOp>();
      if (!nanCmp || nanCmp.getPredicate() != arith::CmpFPredicate::UNE ||
          nanCmp.getLhs() != cmpOp.getLhs() ||
          nanCmp.getRhs() != cmpOp.getLhs()) {
        return failure();
      }
      if (!hasSingleUseBy(nanCmp.getResult(), orOp)) {
        return failure();
      }
...
public:
  LogicalResult matchAndRewrite(CmpOp cmpOp,
    ...
    if (failed(matchSimpleMinMax(cmpOp, selectOp)) &&
        failed(matchNaNAwareMinMax(cmpOp, nanCmp, orOp, selectOp))) {
      return failure();
    }
...
public:
  ...
  matchAndRewrite(triton::ScanOp op, OpAdaptor adaptor,
    ...
    if (adaptor.getOperands().size() != 1) {
      return rewriter.notifyMatchFailure(
          op, "Only support cumsum scan with exactly one operand");
    }
    ...
    if (!type) {
      return rewriter.notifyMatchFailure(op,
                                         "Only support ranked tensor input");
    }
    ...
    if (!zeroAttr) {
      return rewriter.notifyMatchFailure(
          op, "Only support integer/index/float element types");
    }

    Value c0 = rewriter.create<arith::ConstantIndexOp>(loc, 0);
    Value c1 = rewriter.create<arith::ConstantIndexOp>(loc, 1);
    Value zero = rewriter.create<arith::ConstantOp>(loc, elementType, zeroAttr);
    Value init = rewriter.create<tensor::EmptyOp>(loc, type.getShape(), elementType);

    const bool isFloatType = isa<FloatType>(elementType);
    const bool isIntLikeType = isa<IntegerType, IndexType>(elementType);
    if (!isFloatType && !isIntLikeType) {
      return rewriter.notifyMatchFailure(
          op, "Only support integer/index/float element types");
    }

    if (type.getRank() == 1) {
      auto upper = rewriter.create<tensor::DimOp>(loc, input, 0);
      auto loop = rewriter.create<scf::ForOp>(loc, c0, upper, c1,
                                              ValueRange{zero, init},
                                              [&](OpBuilder &b, Location bodyLoc,
                                                  Value i, ValueRange iterArgs) {
                                                Value acc = iterArgs[0];
                                                Value out = iterArgs[1];
                                                Value elem = b.create<tensor::ExtractOp>(
                                                    bodyLoc, input, ValueRange{i});
                                                Value sum = isFloatType
                                                                ...
                                                                : b.create<arith::AddIOp>(
                                                                      bodyLoc, acc, elem)
                                                                      .getResult();
                                                Value outNext = b.create<tensor::InsertOp>(
                                                    bodyLoc, sum, out, ValueRange{i});
                                                b.create<scf::YieldOp>(
                                                    bodyLoc,
                                                    ValueRange{sum, outNext});
                                              });
      rewriter.replaceOp(op, loop->getResult(1));
      return success();
    }

    auto dim0 = rewriter.create<tensor::DimOp>(loc, input, 0);
    auto dim1 = rewriter.create<tensor::DimOp>(loc, input, 1);
    auto outer = rewriter.create<scf::ForOp>(
        loc, c0, dim0, c1, ValueRange{init},
        [&](OpBuilder &bOuter, Location outerLoc, Value i,
            ValueRange outerIterArgs) {
          Value outIter = outerIterArgs[0];
          auto inner = bOuter.create<scf::ForOp>(
              outerLoc, c0, dim1, c1, ValueRange{zero, outIter},
              [&](OpBuilder &bInner, Location innerLoc, Value j,
                  ValueRange innerIterArgs) {
                Value acc = innerIterArgs[0];
                Value out = innerIterArgs[1];
                Value elem = bInner.create<tensor::ExtractOp>(
                    innerLoc, input, ValueRange{i, j});
                Value sum = isFloatType
                                ? bInner.create<arith::AddFOp>(innerLoc, acc, elem)
                                      .getResult()
                                : bInner.create<arith::AddIOp>(innerLoc, acc, elem)
                                      .getResult();
                Value outNext = bInner.create<tensor::InsertOp>(
                    innerLoc, sum, out, ValueRange{i, j});
                bInner.create<scf::YieldOp>(innerLoc,
                                            ValueRange{sum, outNext});
              });
          bOuter.create<scf::YieldOp>(outerLoc, ValueRange{inner->getResult(1)});
        });
