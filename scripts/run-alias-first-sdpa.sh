...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/run-alias-first-sdpa.sh \
    --mode=[on|off|default|comma-separated-list] \
    --model=[gpt2|gpt-neo|opt|pythia|default|comma-separated-list] \
    --artifact-dir=<path>

Default:
  Compile all four LLMs at sequence length 256 with SDPA and alias-first both
  enabled and disabled. Artifacts use the fp32 8x8 Gemmini backend and request
  only a 4-core ELF.

Options:
  --mode=LIST         Alias-first modes. Default: on,off.
  --model=LIST        LLM models. Default: gpt2,gpt-neo,opt,pythia.
...
# Keep the comparison focused on the alias-first pass. Optional custom-op and
# external-call lowerings could otherwise change the generated kernel set.
...
for model in "${models[@]}"; do
  ...
  for mode in "${modes[@]}"; do
    ...
    suffix="${model}/${backend}/sdpa/seq${seq_len}/alias-first-${mode}"
    ...
    cache_key="alias-first-sdpa-${model}-${backend}-seq${seq_len}-${mode}"
    ...
    pc_run_compile_once "${backend}" "${output_dir}" "${cache_key}" \
      "${script_path}" "${compile_args[@]}"
  done
done

pc_log "done; alias-first SDPA artifacts are under ${PC_REPO_ROOT}/examples"
