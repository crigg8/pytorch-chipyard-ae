    ...
    def do_bench(
        ...
        *input_tensors: torch.Tensor,
        ...
        if config.cpu_backend == "triton_chipyard":
            return benchmarker.benchmark_cpu(fn, warmup=0)
...
class TritonBenchmarkRequest(BenchmarkRequest):
    ...
    def make_run_fn(
        ...
        input_tensors_for_run = input_tensors
        if out.device.type == "cpu" and config.cpu_backend == "triton_chipyard":
            autotuner = run_method.__self__
            if not autotuner.launchers:
                autotuner.precompile()
            if autotuner.launchers:
                launcher = autotuner.launchers[0]
                positional_params = [
                    param
                    for param in inspect.signature(launcher).parameters.values()
                    if param.kind
                    in (
                ...
                if positional_params and positional_params[-1].name == "stream":
                    launcher_arg_count = len(positional_params) - 1
                    expected_input_count = launcher_arg_count - 1 - len(extra_args)
                    if 0 <= expected_input_count <= len(input_tensors):
                        input_tensors_for_run = input_tensors[:expected_input_count]
                    else:
                        autotuning_log.debug(
                            "chipyard benchmark arg mismatch: launcher expects %d tensor inputs, got %d; keeping all inputs",
