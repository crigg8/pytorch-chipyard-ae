...
def _is_materialized_lifted_tensor(value: Any) -> bool:
...
def _materialized_lifted_tensors(values: Any) -> tuple[torch.Tensor, ...]:
    return tuple(
        value for value in (values or ()) if _is_materialized_lifted_tensor(value)
    )


def _preserved_tracing_context_lifted_tensors() -> tuple[torch.Tensor, ...]:
    tracing_context = torch._guards.TracingContext.try_get()
    if tracing_context is None:
        return ()

    for attr_name in ("params_flat", "params_flat_unwrap_subclasses"):
        preserved = _materialized_lifted_tensors(
            getattr(tracing_context, attr_name, ()) or ()
        )
        if preserved:
            return preserved
    return ()
...
class _CompileFxKwargs(TypedDict, total=False):
    ...
    lifted_input_tensors: Sequence[torch.Tensor]
    is_backward: bool
...
def compile_fx_inner(
    ...
    **kwargs: Unpack[_CompileFxKwargs],
    ...
    kwargs.setdefault("lifted_input_tensors", ())
...
class _InProcessFxCompile(FxCompile):
    ...
    def codegen_and_compile(
        ...
        graph_lifted_input_tensors: Sequence[torch.Tensor] = graph_kwargs.get(
            "lifted_input_tensors", ()
        )
        ...
        tracing_context = torch._guards.TracingContext.try_get()
        preserved_lifted_input_tensors = _materialized_lifted_tensors(
            graph_lifted_input_tensors
        )
        if not preserved_lifted_input_tensors and tracing_context is not None:
            preserved_lifted_input_tensors = _preserved_tracing_context_lifted_tensors()
        if not preserved_lifted_input_tensors and static_input_idxs:
            preserved_lifted_input_tensors = _materialized_lifted_tensors(
                tensor
        ...
        with (
            ...
            with (
                ...
                graph = GraphLowering(
                    ...
                    lifted_input_tensors=preserved_lifted_input_tensors,
                )
...
def compile_fx(
    ...
    with (
        ...
        preserved_model_lifted_input_tensors: tuple[torch.Tensor, ...] = ()
        if isinstance(model_, torch.nn.Module):
            preserved_model_lifted_input_tensors = _materialized_lifted_tensors(
                tensor
                for _name, tensor in itertools.chain(
                    model_.named_parameters(remove_duplicate=False),
                    model_.named_buffers(remove_duplicate=False),
                )
            )
        if not preserved_model_lifted_input_tensors:
            preserved_model_lifted_input_tensors = (
                _preserved_tracing_context_lifted_tensors()
            )
        if not preserved_model_lifted_input_tensors:
            tracing_context = torch._guards.TracingContext.try_get()
            static_input_idxs = tuple(
                getattr(getattr(tracing_context, "fw_metadata", None), "static_input_indices", ())
                or ()
            )
            if static_input_idxs:
                preserved_model_lifted_input_tensors = (
                    _materialized_lifted_tensors(
                        tensor
                        for index, tensor in enumerate(example_inputs_)
                        if index in static_input_idxs
                    )
                )
        ...
        def fw_compiler_base(
            ...
            with dynamo_utils.dynamo_timed("compile_fx.<locals>.fw_compiler_base"):
                ...
                lifted_input_tensors = preserved_model_lifted_input_tensors
                if len(lifted_input_tensors) != len(static_input_idxs):
                    lifted_input_tensors = _preserved_tracing_context_lifted_tensors()
                if len(lifted_input_tensors) != len(static_input_idxs):
                    lifted_input_tensors = _materialized_lifted_tensors(
                        tensor
                        for index, tensor in enumerate(example_inputs)
                        if index in static_input_idxs
                    )
                ...
                return inner_compile(
                    ...
                    example_inputs,
                    static_input_idxs=static_input_idxs,
                    lifted_input_tensors=lifted_input_tensors,
                    cudagraphs=cudagraphs,
                    graph_id=graph_id,
                    is_inference=is_inference,
                    boxed_forward_device_index=forward_device,
