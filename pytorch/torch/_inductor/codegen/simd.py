...
class SIMDScheduling(BaseScheduling):
    ...
    def _chipyard_runner_codegen_enabled() -> bool:
        return (
            config.cpu_backend == "triton_chipyard"
            and os.environ.get("TORCHINDUCTOR_ENABLE_CHIPYARD_RUNNER", "") == "1"
        )

    @staticmethod
    @contextlib.contextmanager
    def _swap_template_make_kernel_render(template_buffer, make_kernel_render):
        prior_make_kernel_render = getattr(template_buffer, "make_kernel_render", None)
        template_buffer.make_kernel_render = make_kernel_render
        try:
            yield
        finally:
            template_buffer.make_kernel_render = prior_make_kernel_render

    def _emit_chipyard_template_variants(
        self,
        template_node,
        epilogue_nodes,
        prologue_nodes,
        ...
        if not self._chipyard_runner_codegen_enabled():
            return

        site_id = str(
            getattr(template_node.node, "chipyard_autotune_site_id", "") or ""
        )
        if not site_id:
            return
        ...
        if not isinstance(site_entries_by_id, dict) or not isinstance(
            choice_callers_by_site, dict
        ):
            return

        site_entry = site_entries_by_id.get(site_id)
        choice_callers = choice_callers_by_site.get(site_id)
        if not isinstance(site_entry, dict) or not isinstance(choice_callers, list):
            return
        if bool(site_entry.get("wrapper_variants_emitted", False)):
            return

        site_choices = site_entry.get("choices", [])
        if not isinstance(site_choices, list) or not site_choices:
            return

        raw_default_choice_index = site_entry.get("default_choice_index", 0)
        ...
        site_entry["wrapper_variants_emitted"] = True

        if 0 <= default_choice_index < len(site_choices):
            default_choice = site_choices[default_choice_index]
            if isinstance(default_choice, dict):
                default_choice["wrapper_kernel_name"] = default_kernel_name
                default_choice["dispatch_kind"] = "full_wrapper_variant"

        for choice_index, choice_entry in enumerate(site_choices):
            if not isinstance(choice_entry, dict):
                continue
            if choice_index == default_choice_index:
                continue
            if choice_index >= len(choice_callers):
                choice_entry["wrapper_generation_error"] = (
                    "missing choice caller for deferred autotune candidate"
                )
                continue

            choice_caller = choice_callers[choice_index]
            get_make_kernel_render = getattr(choice_caller, "get_make_kernel_render", None)
            if not callable(get_make_kernel_render):
                choice_entry["wrapper_generation_error"] = (
                    "choice caller does not support template wrapper regeneration"
                )
                continue

            try:
                with self._swap_template_make_kernel_render(
                    template_node.node, get_make_kernel_render()
                ):
                    variant_src_code = self.codegen_template(
                        template_node,
                        epilogue_nodes,
                        prologue_nodes,
                        only_gen_src_code=True,
                    )
                if not isinstance(variant_src_code, str) or not variant_src_code.strip():
                    choice_entry["wrapper_generation_error"] = (
                        "generated empty wrapper source for deferred autotune candidate"
                    )
                    continue
                variant_kernel_name = self.define_kernel(
                    variant_src_code, node_schedule, kernel
                )
            except Exception as exc:
                choice_entry["wrapper_generation_error"] = repr(exc)
                log.warning(
                    "Chipyard deferred autotune wrapper generation failed for site %s "
                    "candidate %s: %s",
                    site_id,
                    choice_index,
                    exc,
            ...
            choice_entry["wrapper_kernel_name"] = variant_kernel_name
            choice_entry["dispatch_kind"] = "full_wrapper_variant"
    ...
    def codegen_template(
        ...
        with V.set_kernel_handler(kernel):
            ...
            kernel_name = self.define_kernel(src_code, node_schedule, kernel)
            self._emit_chipyard_template_variants(
                template_node,
                epilogue_nodes,
                prologue_nodes,
                node_schedule,
