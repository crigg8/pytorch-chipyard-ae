...
class TritonSharedOverrides(TritonOverrides):
    _LOG_E_10 = 2.3025850929940456840179914546844
    ...
    @maybe_upcast_float32()
    def cosh(x):
        return f"((tl.exp({x}) + tl.exp(-({x}))) * 0.5)"
    ...
    @maybe_upcast_float32()
    def copysign(x, y):
        return f"tl.where(({y}) < 0, -tl.abs({x}), tl.abs({x}))"
    ...
    @maybe_upcast_float32()
    def hypot(x, y):
        return f"tl.sqrt(({x})*({x}) + ({y})*({y}))"

    @staticmethod
    @maybe_upcast_float32()
    def log10(x):
        return f"(tl.log({x}) / {TritonSharedOverrides._LOG_E_10})"
    ...
    @maybe_upcast_float32()
    def log1p(x):
        return f"tl.log(({x}) + 1.0)"

    @staticmethod
    @maybe_upcast_float32()
    def tan(x):
        return f"(tl.sin({x}) / tl.cos({x}))"

    @staticmethod
    @maybe_upcast_float32()
    def tanh(x):
        return f"((1.0 - tl.exp(-2.0*({x}))) / (1.0 + tl.exp(-2.0*({x}))))"
    ...
    @maybe_upcast_float32()
    def round(x):
        return f"tl.where(({x}) >= 0, tl.floor(({x}) + 0.5), tl.ceil(({x}) - 0.5))"
    ...
    @maybe_upcast_float32()
    def trunc(x):
        # trunc(x) = floor(x) for x >= 0 else ceil(x)
        return f"tl.where(({x}) >= 0, tl.floor({x}), tl.ceil({x}))"
...
class TritonSharedKernelOverrides(TritonKernelOverrides):
    exp = TritonSharedOverrides.__dict__['exp']
    exp2 = TritonSharedOverrides.__dict__['exp2']
    expm1 = TritonSharedOverrides.__dict__['expm1']
    sqrt = TritonSharedOverrides.__dict__['sqrt']
    erf = TritonSharedOverrides.__dict__['erf']
    cosh = TritonSharedOverrides.__dict__['cosh']
    sinh = TritonSharedOverrides.__dict__['sinh']
    #todo: acos
    #todo: acosh
    #todo: asin
    #todo: asinh
    ...
    #todo: atan
    #todo: atanh
    copysign = TritonSharedOverrides.__dict__['copysign']
    #todo: erfc
    #todo: arfinv
    hypot = TritonSharedOverrides.__dict__['hypot']
    log10 = TritonSharedOverrides.__dict__['log10']
    log2 = TritonSharedOverrides.__dict__['log2']
    #todo: nextafter
    rsqrt = TritonSharedOverrides.__dict__['rsqrt']
    log1p = TritonSharedOverrides.__dict__['log1p']
    tan = TritonSharedOverrides.__dict__['tan']
    tanh = TritonSharedOverrides.__dict__['tanh']
    signbit = TritonSharedOverrides.__dict__['signbit']
    fmod = TritonSharedOverrides.__dict__['fmod']
    #todo: pow
    #todo: isinf
    #todo: isnan
    round = TritonSharedOverrides.__dict__['round']
    floor = TritonSharedOverrides.__dict__['floor']
    trunc = TritonSharedOverrides.__dict__['trunc']
    ceil = TritonSharedOverrides.__dict__['ceil']


class TritonChipyardOverrides(TritonOverrides):
    _LOG_E_10 = 2.3025850929940456840179914546844
    ...
    @maybe_upcast_float32()
    def log10(x):
        return f"(tl.log({x}) / {TritonChipyardOverrides._LOG_E_10})"
    ...
    def tanh(x):
        sign = f"(({x}) < 0.0)"
        a = f"tl.where({sign}, -({x}), ({x}))"
        e = TritonChipyardOverrides.exp(f"(-2.0 * {a})")
        y = f"((1.0 - {e}) / (1.0 + {e}))"
        return f"tl.where({sign}, -({y}), {y})"
...
class TritonChipyardKernelOverrides(TritonKernelOverrides):
    exp = TritonChipyardOverrides.__dict__['exp']
    exp2 = TritonChipyardOverrides.__dict__['exp2']
    expm1 = TritonChipyardOverrides.__dict__['expm1']
    sqrt = TritonChipyardOverrides.__dict__['sqrt']
    erf = TritonChipyardOverrides.__dict__['erf']
    cosh = TritonChipyardOverrides.__dict__['cosh']
    sinh = TritonChipyardOverrides.__dict__['sinh']
    #todo: acos
    #todo: acosh
    #todo: asin
    #todo: asinh
    ...
    #todo: atan
    #todo: atanh
    copysign = TritonChipyardOverrides.__dict__['copysign']
    #todo: erfc
    #todo: arfinv
    hypot = TritonChipyardOverrides.__dict__['hypot']
    log10 = TritonChipyardOverrides.__dict__['log10']
    log2 = TritonChipyardOverrides.__dict__['log2']
    #todo: nextafter
    rsqrt = TritonChipyardOverrides.__dict__['rsqrt']
    log1p = TritonChipyardOverrides.__dict__['log1p']
    tan = TritonChipyardOverrides.__dict__['tan']
    tanh = TritonChipyardOverrides.__dict__['tanh']
    signbit = TritonChipyardOverrides.__dict__['signbit']
    fmod = TritonChipyardOverrides.__dict__['fmod']
    #todo: pow
    #todo: isinf
    #todo: isnan
    round = TritonChipyardOverrides.__dict__['round']
    floor = TritonChipyardOverrides.__dict__['floor']
    trunc = TritonChipyardOverrides.__dict__['trunc']
    ceil = TritonChipyardOverrides.__dict__['ceil']


def _use_triton_shared_cpu_overrides() -> bool:
    try:
        dev = V.graph.get_current_device_or_throw()
    except Exception:
        return False
    return dev.type == "cpu" and config.cpu_backend == "triton_shared"


def _use_triton_chipyard_cpu_overrides() -> bool:
    try:
        dev = V.graph.get_current_device_or_throw()
    except Exception:
        return False
    return dev.type == "cpu" and config.cpu_backend == "triton_chipyard"
...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if _use_triton_shared_cpu_overrides():
            self.overrides = TritonSharedKernelOverrides
        if _use_triton_chipyard_cpu_overrides():
            self.overrides = TritonChipyardKernelOverrides
    ...
    def codegen_block_ptr(
        ...
        if not check:
            ...
            assert other in (", other=0.0", ", other=0")
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        if indexing.has_mask():
            if self._load_other:
                ...
                other = f", other={constant_repr(_masked_load_other_value(V.graph.get_dtype(name)))}"
        else:
            other = ""
    ...
    def call_kernel(self, name: str, node: Optional[IRNode] = None):
        ...
        call_triton_meta = self.triton_meta
        if config.cpu_backend == "triton_chipyard":
            inplace_aliases: dict[str, str] = {}
            seen_inplaced: set[int] = set()
            for inplaced in self.args.inplace_buffers.values():
                if isinstance(inplaced, RemovedArg):
                    continue
                inplaced_id = id(inplaced)
                if inplaced_id in seen_inplaced:
                    continue
                seen_inplaced.add(inplaced_id)
                other_names = [str(name) for name in inplaced.other_names if name]
                if len(other_names) >= 2:
                    inplace_aliases[other_names[-1]] = other_names[-2]
            if inplace_aliases:
                call_triton_meta = dict(self.triton_meta)
                call_triton_meta["chipyard_inplace_aliases"] = inplace_aliases
