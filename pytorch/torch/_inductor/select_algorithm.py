...
def _masked_load_other_value(dtype: torch.dtype) -> Union[int, float]:
    return 0 if dtype in (torch.int8, torch.uint8, torch.int16, torch.uint16, torch.int32, torch.int64, torch.bool) else 0.0

# correctness checks struggle with fp16/tf32
VERIFY: dict[str, Any] = {}
PRINT_AUTOTUNE = True
DEBUG = False
TRITON_TEMPLATE_BACKEND_ONLY_META_KEYS = frozenset(
    ("gemmini_tile_i", "gemmini_tile_j", "gemmini_tile_k")
)


def _is_triton_chipyard_cpu_backend(device: Optional[torch.device] = None) -> bool:
    if config.cpu_backend != "triton_chipyard":
        return False
    if device is None:
        return True
...
def _get_chipyard_last_kernel_cycles() -> Optional[int]:
    try:
        import triton
    except ImportError:
        return None
    ...
    try:
        getter = getattr(triton.runtime.driver.active.utils, "get_last_kernel_cycles", None)
    except Exception:
        return None
...
def _benchmark_chipyard_cycles(
    fn: Callable[[], None],
    warmup_iters: int = 1,
    ...
    for _ in range(benchmark_iters):
        fn()
        cycles = _get_chipyard_last_kernel_cycles()
        if cycles is None:
            return benchmarker.benchmark_cpu(fn)
        timings.append(float(cycles))

    return median(timings)


def _choice_timing_key(choice, timings) -> tuple[Any, ...]:
    timing = timings[choice]
    return (timing, choice.hash_key())


def _timing_unit(choices: Sequence[Any]) -> str:
    if choices and all(
        isinstance(choice, TritonTemplateCaller)
        and _is_triton_chipyard_cpu_backend(choice.layout.device)
...
def _chipyard_sympy_seq(values: Sequence[Any]) -> list[str]:
    return [sympy.sstr(value) for value in values]


def _chipyard_stable_kernel_name(prefix: str, signature_parts: Sequence[Any]) -> str:
    stable_hash = hashlib.sha256(
        repr(tuple(signature_parts)).encode("utf-8")
    ).hexdigest()[:12]
    return f"{prefix}_{stable_hash}"


def _chipyard_irnode_summary(node: Any) -> dict[str, Any]:
    summary: dict[str, Any] = {"type": type(node).__name__}
    if hasattr(node, "get_name"):
        summary["name"] = str(node.get_name())
    if hasattr(node, "get_dtype"):
        summary["dtype"] = str(node.get_dtype())
    if hasattr(node, "get_size"):
        summary["size"] = _chipyard_sympy_seq(node.get_size())
    if hasattr(node, "get_stride"):
        summary["stride"] = _chipyard_sympy_seq(node.get_stride())
    layout = getattr(node, "layout", None)
    offset = getattr(layout, "offset", None)
    if offset is not None:
        summary["offset"] = sympy.sstr(offset)
    return summary


def _chipyard_semantic_irnode_summary(node: Any) -> dict[str, Any]:
    summary = _chipyard_irnode_summary(node)
    summary.pop("name", None)
    return summary


def _chipyard_choice_semantic_summary(choice: Any) -> dict[str, Any]:
    summary: dict[str, Any] = {
        "type": type(choice).__name__,
        "hash_key": str(choice.hash_key()),
    }
    kernel_hash_key = getattr(choice, "kernel_hash_key", None)
    if callable(kernel_hash_key):
        try:
            summary["kernel_hash_key"] = str(kernel_hash_key())
        except Exception:
            pass
    if isinstance(choice, TritonTemplateCaller):
        summary["kernel_name"] = str(choice.bmreq.kernel_name)
        summary["num_stages"] = int(choice.bmreq.num_stages)
        summary["num_warps"] = int(choice.bmreq.num_warps)
        summary["extra_args"] = [
            sympy.sstr(arg) if isinstance(arg, sympy.Basic) else repr(arg)
            for arg in choice.bmreq.extra_args
        ]
    return summary


def _chipyard_deferred_autotune_site_key(
    name: str,
    choices: list[ChoiceCaller],
    input_nodes: Any,
    layout: Any,
) -> str:
    layout_summary = _chipyard_semantic_irnode_summary(layout)
    layout_summary["device"] = str(layout.device)
    key_payload = {
        "name": str(name),
        "device": str(layout.device),
        "layout": layout_summary,
        "inputs": [_chipyard_semantic_irnode_summary(node) for node in input_nodes],
        "choices": [_chipyard_choice_semantic_summary(choice) for choice in choices],
    }
...
class TritonTemplateKernel(TritonKernel):
    ...
    def jit_lines(self):
        ...
        for key in ("gemmini_tile_i", "gemmini_tile_j", "gemmini_tile_k"):
            value = self.meta.get(key, None)
            if value is not None:
                triton_meta[key] = value
        chipyard_autotune_site_id = getattr(
            self.output_node, "chipyard_autotune_site_id", None
        )
        if chipyard_autotune_site_id is not None:
            triton_meta["chipyard_autotune_site_id"] = str(chipyard_autotune_site_id)
        chipyard_default_choice_name = getattr(
            self.output_node, "chipyard_default_choice_name", None
        )
        if chipyard_default_choice_name is not None:
            triton_meta["chipyard_default_choice_name"] = str(
                chipyard_default_choice_name
            )
        chipyard_default_kernel_name = getattr(
            self.output_node, "chipyard_default_kernel_name", None
        )
        if chipyard_default_kernel_name is not None:
            triton_meta["chipyard_default_kernel_name"] = str(
    ...
    def load_input(
        ...
        mask: Optional[str] = None,
        other: Optional[Union[float, int]] = None,
        indent_width: int = 4,
    ):
        ...
        Args:
            ...
            other (Optional[Union[float, int]]): The value to use for masked elements. Default is dtype-aware zero.
        ...
        with self.create_subgraph_body(f"<LOAD_INPUT_{input_name}>"):
                ...
                def store(
                    ...
                    if name in V.kernel.prologue_fused_inputs:
                        ...
                        if template_mask != "None" and (
                            ...
                            value_str = (
                                f"tl.where({template_mask}, {value_str}, {masked_other})"
                            )
            ...
            # Generate load code
            masked_other = _masked_load_other_value(input_node.get_dtype()) if other is None else other
            load_code = f"{output_name} = tl.load({input_name} + ({output_index_str})"

            if mask:
                load_code += f", mask={mask}, other={masked_other})"
            else:
                load_code += ")"
    ...
    def make_load(self, name, indices, mask):
        ...
        other = _masked_load_other_value(self.named_input_nodes[name].get_dtype())
        return f"tl.load({name} + ({index}), {mask}, other={other})"
...
class TritonTemplate(KernelTemplate):
    ...
    def generate_and_load(
        ...
        for name, val in kwargs.items():
            if name in TRITON_TEMPLATE_BACKEND_ONLY_META_KEYS:
                continue
            defines.write(f"{name} : tl.constexpr = {val}\n")
        defines = defines.getvalue()

        fake_out = ir.Buffer(name="buf_out", layout=layout)
        kernel_prefix = f"triton_{self.name}"
        kernel_name = kernel_prefix
        if _is_triton_chipyard_cpu_backend(layout.device):
            kernel_name = _chipyard_stable_kernel_name(
                kernel_prefix,
                (
                    tuple(
                        (str(key), repr(value))
                    ...
                    suffix_args,
                    epilogue_fn_hash,
                    workspace_arg.count if workspace_arg is not None else None,
                    workspace_arg.zero_mode.name if workspace_arg is not None else None,
                    _chipyard_irnode_summary(fake_out),
                    tuple(_chipyard_irnode_summary(node) for node in input_nodes),
                    tuple(_chipyard_sympy_seq(call_sizes)),
                ),
            )
        ...
        def generate_code(kernel) -> Optional[tuple[str, str]]:
            def make_extra() -> str:
                ...
                if not _is_triton_chipyard_cpu_backend(layout.device):
                    extra_parts.extend(
                        [
    ...
    def generate(  # type: ignore[override]
        ...
        **kwargs,
        ...
        kernel_hash_name = result.kernel_name
        ...
        block_m = kwargs.get("BLOCK_M", -1)
        block_k = kwargs.get("BLOCK_K", -1)
        block_n = kwargs.get("BLOCK_N", -1)
        log_info: dict[str, Union[PrimitiveInfoType, list[PrimitiveInfoType]]] = {
            "tile_shape": str((block_m, block_k, block_n)),
        ...
        if not _is_triton_chipyard_cpu_backend(layout.device):
            log_info["num_stages"] = num_stages
            log_info["num_warps"] = num_warps
        if result.uses_tl_dot:
            log_info["uses_tl_dot"] = True
            log_info["dot_operand_dtypes"] = [
                str(node.get_dtype()) for node in full_input_nodes[:2]
            ]
            for key in ("gemmini_tile_i", "gemmini_tile_j", "gemmini_tile_k"):
                if kwargs.get(key) is not None:
                    log_info[key] = int(kwargs[key])
...
class TritonTemplateCaller(ir.TritonTemplateCallerBase):
    def __init__(
        ...
        self.log_info["backend"] = "Triton"
        if not _is_triton_chipyard_cpu_backend(self.layout.device):
            self.log_info.setdefault("num_stages", self.bmreq.num_stages)
            self.log_info.setdefault("num_warps", self.bmreq.num_warps)
        self.mutated_inputs = mutated_inputs
        self.workspace_arg = workspace_arg
        self.allowed_prologue_inps = (
            allowed_prologue_inps if allowed_prologue_inps is not None else OrderedSet()
        )
        self.chipyard_autotune_site_id: Optional[str] = None
    ...
    def benchmark(self, *args, out):
        ...
        if _is_triton_chipyard_cpu_backend(self.layout.device):
            try:
                algo = self.bmreq.make_run_fn(*args, out=out)
            except NonzeroWorkspaceNotSupportedError:
                return float("inf")
            try:
                return _benchmark_chipyard_cycles(algo)
            finally:
                self.bmreq.cleanup_run_fn()
    ...
    def output_node(self):
        template_buffer = ir.TritonTemplateBuffer(
            layout=self.layout,
        ...
        if self.chipyard_autotune_site_id is not None:
            template_buffer.chipyard_autotune_site_id = self.chipyard_autotune_site_id
            template_buffer.chipyard_default_choice_name = self.name
            template_buffer.chipyard_default_kernel_name = self.bmreq.kernel_name
        return ir.TensorBox.create(template_buffer)
    ...
    def autoheuristic_id(self):
        ...
        BLOCK_N = tile_vals[2]
        suffix = ""
        if "num_stages" in info and "num_warps" in info:
            num_stages = info["num_stages"]
            num_warps = info["num_warps"]
            suffix += f"_numstages={num_stages}_numwarps={num_warps}"
        return (
            f"type={type_name}_BLOCK-M={BLOCK_M}_BLOCK-K={BLOCK_K}_BLOCK-N={BLOCK_N}"
            f"{suffix}"
        )
...
class AlgorithmSelectorCache(PersistentCache):
    ...
    @staticmethod
    def _prepare_chipyard_choices(choices: list[ChoiceCaller]) -> list[ChoiceCaller]:
        if not any(
            isinstance(choice, TritonTemplateCaller)
            and _is_triton_chipyard_cpu_backend(choice.layout.device)
            for choice in choices
        ...
        deduped_choices: list[ChoiceCaller] = []
        seen_hashes: OrderedSet[str] = OrderedSet()
        for choice in choices:
            choice_hash = choice.hash_key()
            if choice_hash in seen_hashes:
                continue
            seen_hashes.add(choice_hash)
            deduped_choices.append(choice)

        return deduped_choices

    @staticmethod
    def _record_chipyard_deferred_autotune_site(
        name: str,
        choices: list[ChoiceCaller],
        input_nodes,
        layout,
        default_choice: ChoiceCaller,
    ) -> Optional[str]:
        if not hasattr(V.graph, "chipyard_deferred_autotune_sites"):
            return None
        site_key = _chipyard_deferred_autotune_site_key(
            name,
            choices,
            input_nodes,
            layout,
        )
        site_entries_by_key = getattr(
            V.graph, "chipyard_deferred_autotune_sites_by_key", None
        )
        if isinstance(site_entries_by_key, dict):
            existing_site_entry = site_entries_by_key.get(site_key)
            if isinstance(existing_site_entry, dict):
                existing_site_id = str(existing_site_entry.get("site_id", ""))
                if existing_site_id:
                    choice_callers_by_site = getattr(
                        V.graph, "chipyard_deferred_autotune_choice_callers", None
                    )
                    if isinstance(choice_callers_by_site, dict):
                        choice_callers_by_site[existing_site_id] = list(choices)
                    return existing_site_id
        site_index = int(
            getattr(V.graph, "chipyard_deferred_autotune_site_counter", 0)
        )
        site_id = f"{site_key}_{site_index}"
        V.graph.chipyard_deferred_autotune_site_counter = site_index + 1
        default_choice_index = next(
            (
                choice_index
        ...
        for choice in choices:
            candidate_entry: dict[str, Any] = {
                "choice_name": str(choice.name),
                "description": str(choice.description),
                "hash_key": str(choice.hash_key()),
                "kernel_hash_key": str(choice.kernel_hash_key()),
                "type": type(choice).__name__,
            }
            if isinstance(choice, TritonTemplateCaller):
                candidate_entry["kernel_name"] = str(choice.bmreq.kernel_name)
                candidate_entry["module_path"] = str(choice.bmreq.module_path)
                candidate_entry["module_cache_key"] = str(choice.bmreq.module_cache_key)
                candidate_entry["num_stages"] = int(choice.bmreq.num_stages)
                candidate_entry["num_warps"] = int(choice.bmreq.num_warps)
                candidate_entry["extra_args"] = [
                    sympy.sstr(arg) if isinstance(arg, sympy.Basic) else repr(arg)
                    for arg in choice.bmreq.extra_args
                ]
                candidate_entry["log_info"] = {
                    str(key): value
                    for key, value in choice.info_dict().items()
                }
            candidate_entries.append(candidate_entry)

        layout_summary = _chipyard_irnode_summary(layout)
        layout_summary["device"] = str(layout.device)
        site_entry = {
            "site_id": site_id,
            "site_key": site_key,
            ...
            "device": str(layout.device),
            "layout": layout_summary,
            "inputs": [_chipyard_irnode_summary(node) for node in input_nodes],
            "default_choice_index": default_choice_index,
            "default_choice_name": str(default_choice.name),
            "default_kernel_name": str(
                default_choice.bmreq.kernel_name
                if isinstance(default_choice, TritonTemplateCaller)
                else default_choice.name
            ),
            "choices": candidate_entries,
        }
        V.graph.chipyard_deferred_autotune_sites.append(site_entry)
        site_entries_by_id = getattr(
            V.graph, "chipyard_deferred_autotune_sites_by_id", None
        )
        if isinstance(site_entries_by_id, dict):
            site_entries_by_id[site_id] = site_entry
        if isinstance(site_entries_by_key, dict):
            site_entries_by_key[site_key] = site_entry
        choice_callers_by_site = getattr(
            V.graph, "chipyard_deferred_autotune_choice_callers", None
        )
        if isinstance(choice_callers_by_site, dict):
            choice_callers_by_site[site_id] = list(choices)
        return site_id
    ...
    def __call__(
        ...
        if _is_triton_chipyard_cpu_backend(layout.device):
            choices = self._prepare_chipyard_choices(choices)
        ...
        if _is_triton_chipyard_cpu_backend(layout.device) and len(choices) > 1:
            default_choice = choices[0]
            site_id = self._record_chipyard_deferred_autotune_site(
                name,
                choices,
                input_nodes,
                layout,
            ...
            if (
                site_id is not None
                and isinstance(default_choice, TritonTemplateCaller)
            ):
                default_choice.chipyard_autotune_site_id = site_id
            node = default_choice.output_node()
            log.debug(
                "Chipyard deferred autotune selected default choice: %s (site_id=%s)",
                node,
                site_id,
            )
            return node
        ...
        if config.autotune_in_subproc and not _is_triton_chipyard_cpu_backend(
            layout.device
        ):
        ...
        choice = min(timings, key=lambda candidate: _choice_timing_key(candidate, timings))
        node = choice.output_node()
    ...
    def log_results(
        ...
        n = config.autotune_num_choices_displayed
        top_k = sorted(timings, key=lambda choice: _choice_timing_key(choice, timings))[:n]
        ...
        def get_choice_info(choice):
            ...
            return {
                ...
                "num_warps": info.get("num_warps"),
                "gemmini_tile_i": info.get("gemmini_tile_i"),
                "gemmini_tile_j": info.get("gemmini_tile_j"),
                "gemmini_tile_k": info.get("gemmini_tile_k"),
                "dot_operand_dtypes": info.get("dot_operand_dtypes", []),
            }
        ...
        for choice in top_k:
            ...
            if result:
                ...
                sys.stderr.write(
                    f"  {choice.name} {result:.4f} {time_unit} {best_time / result:.1%} {kernel_description}\n"
                )
            else:
                sys.stderr.write(
                    f"  {choice.name} {result:.4f} {time_unit} <DIVIDED BY ZERO ERROR>\n"
                )
