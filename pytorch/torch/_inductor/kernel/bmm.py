...
def _is_integer_bmm_dot_input(mat):
...
chipyard_bmm_template = TritonTemplate(
    name="bmm_chipyard",
    grid=bmm_grid,
    source=r"""
{{def_kernel("A", "B")}}
    ...
    for k in range(K, 0, -BLOCK_K):
        if EVEN_K:
            ...
            a = tl.load(A, mask=rk[None, :] < k, other=0.){% if CAST_A_TO_INT8 %}.to(tl.int8){% endif %}
            b = tl.load(B, mask=rk[:, None] < k, other=0.){% if CAST_B_TO_INT8 %}.to(tl.int8){% endif %}
...
def tuned_bmm(mat1, mat2, out_dtype=None, *, layout=None):
    ...
    chipyard_backend = _is_triton_chipyard_backend(layout)
    integer_dot_inputs = chipyard_backend and _is_integer_bmm_dot_input(
        mat1
    ) and _is_integer_bmm_dot_input(mat2)
    cast_a_to_int8 = chipyard_backend and _is_integer_bmm_dot_input(mat1)
    cast_b_to_int8 = chipyard_backend and _is_integer_bmm_dot_input(mat2)
    choices = [aten_func] if use_aten_gemm_kernels() and not integer_dot_inputs else []
    ...
    bmm_configs = (
        ...
        else V.choices.get_base_mm_configs(device_type)
    )
    if use_triton_template(layout, enable_int32=integer_dot_inputs):
        # TODO: add out_dtype support for Triton Template
        assert out_dtype is None, "out_dtype is not supported for Triton"
        selected_bmm_template = (
            chipyard_bmm_template if chipyard_backend else bmm_template
        )
        for config in bmm_configs(
            ...
            **mm_config_kwargs(
                ...
                None if integer_dot_inputs else dtype.itemsize,
            ),
        ):
            selected_bmm_template.maybe_append_choice(
                choices,
...
def tuned_baddbmm(inp, mat1, mat2, *, alpha=1, beta=1, layout=None):
    ...
    if use_triton_template(layout):
        selected_bmm_template = (
            chipyard_bmm_template
            if _is_triton_chipyard_backend(layout)
            else bmm_template
        )
        for config in bmm_configs(
