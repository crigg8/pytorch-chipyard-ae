...
def _is_triton_chipyard_backend(layout) -> bool:
    return (
        ...
        and inductor_config.cpu_backend == "triton_chipyard"
    )


def _is_integer_mm_dot_input(mat):
...
chipyard_mm_template = TritonTemplate(
    name="mm_chipyard",
    grid=mm_grid,
    source=r"""
{{def_kernel("A", "B")}}
...
def tuned_mm(mat1, mat2, *, layout=None):
    ...
    if chipyard_backend and _is_int8_mat(mat1) and _is_int8_mat(mat2):
        return tuned_int_mm(mat1, mat2)
    ...
    integer_dot_inputs = chipyard_backend and _is_integer_mm_dot_input(
        mat1
    ) and _is_integer_mm_dot_input(mat2)
    cast_a_to_int8 = chipyard_backend and _is_integer_mm_dot_input(mat1)
    cast_b_to_int8 = chipyard_backend and _is_integer_mm_dot_input(mat2)
    # options to tune from
    choices = (
        [aten_mm.bind((mat1, mat2), aten_layout)]
        if use_aten_gemm_kernels() and not integer_dot_inputs
        else []
    )
    ...
    triton_template_enabled = use_triton_template(
        layout, enable_int32=integer_dot_inputs
    )
    selected_mm_template = chipyard_mm_template if chipyard_backend else mm_template
    if is_nonzero and triton_template_enabled:
        for config in mm_configs(
            ...
            **mm_config_kwargs(device_type, _is_large_block_for_cpu, dtype.itemsize),
            ...
            selected_mm_template.maybe_append_choice(
                choices,
        ...
        if (
            use_triton_tma_template(mat1, mat2)
            and not integer_dot_inputs
            and not chipyard_backend
        ):
...
def tuned_int_mm(mat1, mat2, *, layout=None):
    ...
    if chipyard_backend:
        choices = []
    elif use_aten_gemm_kernels():
        choices = [aten__int_mm.bind((mat1, mat2), layout)]
    else:
        choices = []

    if not chipyard_backend and use_cutlass and _use_cutlass_for_op("int_mm"):
...
def tuned_scaled_mm(
    ...
    if is_nonzero and use_triton_template(layout, enable_float8=True):
        ...
        if use_triton_tma_template(mat_a, mat_b) and not bias and not chipyard_backend:
