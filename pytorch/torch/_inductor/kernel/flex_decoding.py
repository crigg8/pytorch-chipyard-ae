...
chipyard_flex_decoding_template = TritonTemplate(
    name="flex_decoding_chipyard",
    grid=flex_decoding_grid,
    source=r"""
    {{def_kernel("Q", "K", "V", "M", "L", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX")}}
    # Chipyard flex decoding keeps the split-KV algorithm but avoids CUDA-style
    # block pointers, TMA, and warp-specialization options.
    tl.static_assert(SPARSE_KV_BLOCK_SIZE >= BLOCK_N and SPARSE_KV_BLOCK_SIZE % BLOCK_N == 0)
    ...
    BLOCK_M_PER_HQ: tl.constexpr = BLOCK_M // G
    off_g = tl.arange(0, G)
    offs_g = tl.ravel(tl.broadcast_to(off_g[:, None], [G, BLOCK_M_PER_HQ]))
    offs_hq = offs_g + off_hkv * G
    off_m = tl.arange(0, BLOCK_M_PER_HQ)
    offs_m = tl.ravel(tl.broadcast_to(off_m[None, :], [G, BLOCK_M_PER_HQ]))
    offs_d = tl.arange(0, QK_HEAD_DIM_ROUNDED)
    offs_vd = tl.arange(0, V_HEAD_DIM_ROUNDED)
    ...
    block_n_start = off_t * TILE_KV_MULTIPLE
    block_n_end = block_n_start + TILE_KV_MULTIPLE

    q_range = (
        stride_qg * off_g[:, None, None]
        + stride_qm * off_m[None, :, None]
        + stride_qk * offs_d[None, None, :]
    )

    if not SAFE_M_BOUNDARY and not SAFE_HEAD_DIM:
        q = tl.load(
            Q + q_offset + q_range,
            mask=(offs_d[None, None, :] < QK_HEAD_DIM) & (off_m[None, :, None] < Q_LEN),
            other=0.0,
        )
    elif SAFE_M_BOUNDARY and not SAFE_HEAD_DIM:
        q = tl.load(
            Q + q_offset + q_range,
            mask=offs_d[None, None, :] < QK_HEAD_DIM,
            other=0.0,
        )
    elif not SAFE_M_BOUNDARY and SAFE_HEAD_DIM:
        q = tl.load(
    ...
    acc, l_i, m_i = chipyard_forward_inner(
        {{gen_argdefs()}},
        q, K + k_offset, V + v_offset, Q_LEN, KV_LEN,
        stride_kk, stride_kn, stride_vn, stride_vk,
        acc, l_i, m_i,
        off_z, offs_hq[:, None], offs_m, offs_n,
        kv_indices, kv_num_blocks,
        block_n_start,
        block_n_end if block_n_end <= block_n_last_valid else block_n_last_valid,
        MATMUL_PRECISION,
        IS_FULL_BLOCKS=False,
    )
    ...
    if SAFE_M_BOUNDARY:
        tl.store(M + m_offsets, m_i)
        tl.store(L + l_offsets, l_i)
    else:
        ml_mask = ml_m < Q_LEN
        tl.store(M + m_offsets, m_i, mask=ml_mask)
        tl.store(L + l_offsets, l_i, mask=ml_mask)
...
def create_chipyard_flex_decoding_kernel(*args, **kwargs):
    from .flex_attention import set_head_dim_values
    ...
    for conf in configs:
        ...
        cur_kernel_options.setdefault("BLOCK_N", conf.block_n)
        cur_kernel_options.setdefault("SPARSE_KV_BLOCK_SIZE", SPARSE_KV_BLOCK_SIZE)
        cur_kernel_options["num_warps"] = 1
        cur_kernel_options["num_stages"] = 1
        cur_kernel_options["USE_TMA"] = False
        cur_kernel_options["FLOAT32_PRECISION"] = "'ieee'"

        chipyard_flex_decoding_template.maybe_append_choice(
            choices=choices,
            input_nodes=[
                query,
                key,
