...
def _is_integer_flex_attention_dot_input(t: TensorBox) -> bool:
...
chipyard_compute_next_offset_func = r"""
@triton.jit
def chipyard_get_offset_for_next_block(
    loop_iter, col_indices, total_blocks,
    SPARSE_BLOCK, SPARSE_BLOCK_MULTIPLE, BLOCK,
    BLOCKS_ARE_CONTIGUOUS: tl.constexpr
):
    if BLOCKS_ARE_CONTIGUOUS:
        return BLOCK
    cur_block_idx = loop_iter // SPARSE_BLOCK_MULTIPLE
    cur_block = tl.load(col_indices + cur_block_idx)
    next_block = tl.load(
    ...
    needs_jump = (loop_iter + 1) % SPARSE_BLOCK_MULTIPLE == 0
    jump_to_block = (
        (next_block - cur_block) * SPARSE_BLOCK
        - (SPARSE_BLOCK_MULTIPLE - 1) * BLOCK
    )
    offset = jump_to_block * needs_jump + (1 - needs_jump) * BLOCK
    return offset
"""

chipyard_load_2d_tile = r"""
@triton.jit
def chipyard_load_2d_tile(
    base,
    offs_0,
    offs_1,
    stride_0,
    ...
    ptrs = base + offs_0[:, None] * stride_0 + offs_1[None, :] * stride_1
    mask = (offs_0[:, None] < limit_0) & (offs_1[None, :] < limit_1)
...
def forward_inner(
    ...
    if PRESCALE_QK and not QK_INT_DOT:
...
def forward_block_mn(
    ...
    {%- endif %}
    {%- if USE_FP32_PV_DOT %}
    acc = tl.dot(p.to(tl.float32), v.to(tl.float32), acc, input_precision=FLOAT32_PRECISION)
    {%- else %}
...
chipyard_compute_flex_attention = r"""
{{def_kernel("Q", "K", "V", "LSE", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX")}}
    # Chipyard keeps the FlexAttention kernel signature/layout contract, but
    # uses only raw tl.load/tl.store pointer arithmetic in the forward path.
    tl.static_assert(SPARSE_Q_BLOCK_SIZE >= BLOCK_M and SPARSE_Q_BLOCK_SIZE % BLOCK_M == 0)
    tl.static_assert(SPARSE_KV_BLOCK_SIZE >= BLOCK_N and SPARSE_KV_BLOCK_SIZE % BLOCK_N == 0)
    ...
    offs_m = q_start * BLOCK_M + tl.arange(0, BLOCK_M)
    offs_qk = tl.arange(0, QK_HEAD_DIM_ROUNDED)
    q = chipyard_load_2d_tile(
        Q,
        offs_m,
        offs_qk,
        stride_qm,
    ...
    acc, l_i, m_i = chipyard_forward_inner(
        {{gen_argdefs()}},
        q, K, V, Q_LEN, KV_LEN,
...
chipyard_compute_forward_inner = r"""
@triton.jit
def chipyard_forward_inner(
    {{gen_argdefs()}},
    q, K, V, Q_LEN, KV_LEN,
    stride_kk, stride_kn, stride_vn, stride_vk,
    acc, l_i, m_i,
    ...
    for start_n in range(block_n_start, block_n_end):
        if IS_DIVISIBLE:
            acc, l_i, m_i = chipyard_forward_block_mn(
                {{gen_argdefs()}},
                q, K, V, Q_LEN, KV_LEN,
                stride_kk, stride_kn, stride_vn, stride_vk,
                acc, l_i, m_i,
        ...
        offset = chipyard_get_offset_for_next_block(
            start_n, kv_indices, kv_num_blocks,
            SPARSE_KV_BLOCK_SIZE, SPARSE_KV_MULTIPLE, BLOCK_N, BLOCKS_ARE_CONTIGUOUS
        )
        offs_n = offs_n + offset
...
chipyard_compute_forward_block_mn = r"""
@triton.jit
def chipyard_forward_block_mn(
    {{gen_argdefs()}},
    q, K, V, Q_LEN, KV_LEN,
    stride_kk, stride_kn, stride_vn, stride_vk,
    acc, l_i, m_i,
    ...
    m = get_bounded_indices(offs_m_2d, Q_LEN if CHECK_BLOCK_BOUNDARY else None)
    n = get_bounded_indices(offs_n_2d, KV_LEN if CHECK_BLOCK_BOUNDARY else None)
    ...
    if CHECK_BLOCK_BOUNDARY:
        post_mod_scores = tl.where(offs_n_2d < KV_LEN, post_mod_scores, float("-inf"))
    ...
    if not IS_FULL_BLOCKS:
        ...
        if CHECK_BLOCK_BOUNDARY:
            mask_mod_output = tl.where(offs_n_2d < KV_LEN, mask_mod_output, False)
        post_mod_scores = tl.where(mask_mod_output, post_mod_scores, float("-inf"))
...
chipyard_flex_attention_template = TritonTemplate(
    name="flex_attention_chipyard",
    grid=flex_attention_grid,
    # Chipyard keeps the kernel interface but uses a simpler raw tl.load/store
    # implementation that avoids block pointers, tl.advance, and TMA.
    source=chipyard_compute_flex_attention
    + chipyard_compute_forward_inner
    + chipyard_compute_next_offset_func
    + chipyard_compute_forward_block_mn
    + chipyard_load_2d_tile
    + get_bounded_indices_func,
)


def _use_flex_decoding(query, kv_indices, kernel_options, enable_gqa, *, chipyard=False):
    ...
    short_query_length = V.graph.sizevars.evaluate_expr(
        sympy.Lt(query.get_size()[-2], 32 if chipyard else 128)
    )
...
from torch._inductor.kernel.flex_decoding import (
    create_chipyard_flex_decoding_kernel,
...
_CHIPYARD_FLEX_OPTION_ALLOWLIST = {
...
def _normalize_chipyard_flex_kernel_options(
    kernel_options: dict[str, Any],
) -> dict[str, Any]:
    normalized: dict[str, Any] = {}
    for key, value in kernel_options.items():
        if key.startswith("bwd_"):
            continue
        if key.startswith("fwd_") and key[4:] in _CHIPYARD_FLEX_OPTION_ALLOWLIST:
            normalized[key] = value
            continue
        if key in _CHIPYARD_FLEX_OPTION_ALLOWLIST:
            normalized[key] = value

    normalized.setdefault("PRESCALE_QK", False)
    normalized.setdefault("ROWS_GUARANTEED_SAFE", False)
    normalized.setdefault("BLOCKS_ARE_CONTIGUOUS", False)
    normalized.setdefault("OUTPUT_LOGSUMEXP", False)
    # Chipyard backend currently accepts only IEEE dot precision and does not
    # support the flex-attention TMA path.
    normalized["FLOAT32_PRECISION"] = "'ieee'"
    normalized["USE_TMA"] = False
    return normalized


def lower_triton_template(
    query,
    key,
    value,
    subgraph,
    ...
    # Shared Triton template path for CUDA-style backends.
    ...
    original_kernel_options.setdefault("QK_INT_DOT", False)
    original_kernel_options.setdefault("USE_FP32_PV_DOT", False)
...
def lower_chipyard(
    ...
    kernel_options = _normalize_chipyard_flex_kernel_options(kernel_options)
    if kernel_options["OUTPUT_LOGSUMEXP"]:
        raise NotImplementedError(
            "triton_chipyard flex_attention currently supports inference only."
        )
    ...
    if query.get_dtype() not in [torch.float, torch.bfloat16, torch.float16]:
        raise NotImplementedError(
            "`torch.float`, `torch.float16`, and `torch.bfloat16` are supported "
            "in triton_chipyard FlexAttention."
        )
    if (
        _is_integer_flex_attention_dot_input(query)
        or _is_integer_flex_attention_dot_input(key)
        or _is_integer_flex_attention_dot_input(value)
    ):
        raise NotImplementedError(
            "triton_chipyard flex_attention does not support integer tl.dot inputs yet."
        )
    ...
    if _use_flex_decoding(
        query, kv_indices, kernel_options, enable_gqa, chipyard=True
    ):
        return create_chipyard_flex_decoding_kernel(
            query,
            key,
            value,
            block_mask,
    ...
    configs = V.choices.get_flex_attention_fwd_configs(
        head_dim, dtype, device_type=query.get_device().type
    )
    ...
    for conf in configs:
        ...
        cur_kernel_options.setdefault("BLOCK_M", conf.block_m)
        cur_kernel_options.setdefault("BLOCK_N", conf.block_n)
        cur_kernel_options.setdefault("num_stages", 1)
        cur_kernel_options.setdefault("num_warps", 1)
        cur_kernel_options.setdefault("SPARSE_Q_BLOCK_SIZE", SPARSE_Q_BLOCK_SIZE)
        cur_kernel_options.setdefault("SPARSE_KV_BLOCK_SIZE", SPARSE_KV_BLOCK_SIZE)
        cur_kernel_options["FLOAT32_PRECISION"] = "'ieee'"
        cur_kernel_options["USE_TMA"] = False

        error = chipyard_flex_attention_template.maybe_append_choice(
            choices=choices,
            input_nodes=[
                query,
                key,
