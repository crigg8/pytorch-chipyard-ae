...
from .mm_common import acc_type, mm_config_kwargs
...
def _is_triton_chipyard_backend(device_type: str) -> bool:
    return device_type == "cpu" and config.cpu_backend == "triton_chipyard"


def _use_im2col_mm() -> bool:
    return os.environ.get("TORCHINDUCTOR_IM2COL_MM", "").strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def _uses_int8_conv_accum(x: "TensorBox", weight: "TensorBox") -> bool:
    int8_dtypes = (torch.int8, torch.uint8)
    return x.get_dtype() in int8_dtypes and weight.get_dtype() in int8_dtypes


def _is_integer_conv_dot_input(t: "TensorBox") -> bool:
    return t.get_dtype() in (torch.int8, torch.uint8, torch.int32)


def _conv_masked_load_other_value(t: "TensorBox"):
    return 0 if _is_integer_conv_dot_input(t) else 0.0


def _is_strict_depthwise_conv2d(
    x: "TensorBox",
    *,
    out_chan: int,
    in_chan: int,
    ...
    return (
        ...
        and in_chan == 1
        and out_chan == groups
        and V.graph.sizevars.statically_known_equals(x.get_size()[1], groups)
    )
...
{% if CHIPYARD %}
        matrix_x = tl.load(x_ptrs, mask=mask_x, other={{OTHER_X}}){% if CAST_X_TO_INT8 %}.to(tl.int8){% endif %}
{% else %}
...
{% if CHIPYARD %}
        matrix_w = tl.load(w_ptrs, mask=mask_w, other={{OTHER_W}}){% if CAST_W_TO_INT8 %}.to(tl.int8){% endif %}
        acc = tl.dot(matrix_x, matrix_w, acc, allow_tf32=ALLOW_TF32, out_dtype=ACC_TYPE)
{% else %}
...
{{def_kernel("X", "W")}}
    ...
    idx_y_h = nh % OUT_H
    idx_n = nh // OUT_H
    idx_c = tl.program_id(1) * BLOCK_N + tl.arange(0, BLOCK_N)

    acc = tl.zeros((BLOCK_M, BLOCK_N), dtype=ACC_TYPE)

{% for i in range(KERNEL_H) %}
{% for j in range(KERNEL_W) %}
    idx_x_h = idx_y_h * STRIDE_H + {{i}} * DILATION_H - PADDING_H
    idx_x_w = idx_y_w * STRIDE_W + {{j}} * DILATION_W - PADDING_W

    x_ptrs = X + (
        (idx_n * stride_xn)[:, None]
        + (idx_x_h * stride_xh)[:, None]
        + (idx_x_w * stride_xw)[:, None]
        + (idx_c * stride_xc)[None, :]
    )
    mask_x = (
        (idx_n < BATCH)[:, None]
        & (idx_x_h >= 0)[:, None]
    ...
    matrix_x = tl.load(x_ptrs, mask=mask_x, other={{OTHER_X}}){% if CAST_X_TO_INT8 %}.to(tl.int8){% endif %}
    matrix_x = matrix_x.to(ACC_TYPE)

    w_ptrs = W + idx_c * stride_wc + ({{i}} * stride_wh) + ({{j}} * stride_ww)
    mask_w = idx_c < OUT_C
    matrix_w = tl.load(w_ptrs, mask=mask_w, other={{OTHER_W}}){% if CAST_W_TO_INT8 %}.to(tl.int8){% endif %}
    matrix_w = matrix_w.to(ACC_TYPE)

    acc += matrix_x * matrix_w[None, :]
{% endfor %}
{% endfor %}
...
def conv_layout(
    ...
    output_dtype = (
        torch.int32
        if _is_triton_chipyard_backend(device.type)
        and _is_integer_conv_dot_input(x)
        and _is_integer_conv_dot_input(weight)
        else x.get_dtype()
    )
...
def convert_1x1_conv_to_mm(x, weight, bias, *, stride, padding):
    # Special case for 1x1 convolution: convert it into a matmul.
    chipyard_backend = _is_triton_chipyard_backend(x.get_device_or_error().type)
    ...
    if chipyard_backend and any(padding):
        pad = []
        for amount in reversed(padding):
            pad.extend([amount, amount])
        x = L[aten.constant_pad_nd](x, pad, 0)

    for dim, step in enumerate(stride if chipyard_backend else (), start=2):
        if step != 1:
            x = L[aten.slice](x, dim=dim, step=step)
    ...
    *sizes, in_chan = x.get_size()
    ...
    # Materialize the 2D view so backends that lower mm inputs as raw pointers
    # do not reinterpret the original NCHW storage.
    x = L[aten.clone](x)
    x = ir.ExternKernel.require_contiguous(x)
    if _uses_int8_conv_accum(x, weight):
        result = L[aten._int_mm](x, weight)
        if bias is not None:
            result = L[aten.add](result, bias)
    elif _is_integer_conv_dot_input(x) and _is_integer_conv_dot_input(weight):
        result = L[aten.mm](x, weight)
        if bias is not None:
            result = L[aten.add](result, bias)
    elif bias is None:
...
def convert_conv2d_to_im2col_mm(
    x: TensorBox,
    weight: TensorBox,
    bias: Optional[TensorBox],
    *,
    ...
    batch, out_chan, out_h, out_w = layout.size
    out_image_numel = sympy_product((out_h, out_w))

    out_chan, group_in_chan, *kernel_shape = V.graph.sizevars.evaluate_static_shapes(
        weight.get_size()
    )
    group_k = group_in_chan * sympy_product(kernel_shape)
    ...
    if padding_h or padding_w:
        x = L[aten.constant_pad_nd](
            x, [padding_w, padding_w, padding_h, padding_h], 0
        )

    eff_kernel_h = dilation_h * (kernel_h - 1) + 1
    eff_kernel_w = dilation_w * (kernel_w - 1) + 1

    cols = L[aten.unfold](x, 2, eff_kernel_h, stride_h)
    if dilation_h != 1:
        cols = L[aten.slice](cols, dim=4, step=dilation_h)

    cols = L[aten.unfold](cols, 3, eff_kernel_w, stride_w)
    if dilation_w != 1:
        cols = L[aten.slice](cols, dim=5, step=dilation_w)

    cols = L[aten.permute](cols, [0, 1, 4, 5, 2, 3])
    cols = L[aten.reshape](cols, [batch, group_k * groups, out_image_numel])

    if groups == 1:
        cols = L[aten.permute](cols, [0, 2, 1])
        cols = L[aten.reshape](cols, [batch * out_image_numel, group_k])
        # Materialize the 2D view so GEMM backends read a dense matrix.
        cols = L[aten.clone](cols)
        cols = ir.ExternKernel.require_contiguous(cols)

        weight = L[aten.reshape](weight, [out_chan, group_k])
        weight = L[aten.permute](weight, [1, 0])
        weight = ir.ExternKernel.require_contiguous(weight)

        if bias is None:
            result = L[aten.mm](cols, weight)
        else:
            result = L[aten.addmm](bias, cols, weight)
        result = L[aten.reshape](result, [batch, out_h, out_w, out_chan])
        return L[aten.permute](result, [0, 3, 1, 2])

    out_chan_per_group = out_chan // groups

    cols = L[aten.reshape](cols, [batch, groups, group_k, out_image_numel])
    cols = L[aten.permute](cols, [1, 0, 3, 2])
    cols = L[aten.reshape](cols, [groups, batch * out_image_numel, group_k])
    # Materialize the batched GEMM input so grouped conv lowers cleanly to bmm.
    cols = L[aten.clone](cols)
    cols = ir.ExternKernel.require_contiguous(cols)

    weight = L[aten.reshape](weight, [groups, out_chan_per_group, group_k])
    weight = L[aten.permute](weight, [0, 2, 1])
    weight = ir.ExternKernel.require_contiguous(weight)

    result = L[aten.bmm](cols, weight)
    result = L[aten.reshape](
        result, [groups, batch, out_image_numel, out_chan_per_group]
    )
    result = L[aten.permute](result, [1, 0, 3, 2])
    result = L[aten.reshape](result, [batch, out_chan, out_h, out_w])
    if bias is not None:
        result = L[aten.add](result, L[aten.view](bias, [1, out_chan, 1, 1]))
    return result
...
def convolution(
    ...
    is_strict_depthwise = _is_strict_depthwise_conv2d(
        x, out_chan=out_chan, in_chan=in_chan, ndim=ndim, groups=groups
    )
    chipyard_backend = _is_triton_chipyard_backend(device_type)
    ...
    if (
        (
            chipyard_backend
            or config.conv_1x1_as_mm
            or (autotuning_gemm and channels_last_conv())
        )
        and is_ones(kernel_shape)
        and (chipyard_backend or is_ones(stride))
        and (chipyard_backend or is_zeros(padding))
        and (chipyard_backend or is_ones(dilation))
        and not transposed
        and is_zeros(output_padding)
        and groups == 1
        and V.graph.sizevars.statically_known_gt(sympy_product(x.get_size()), 0)
    ):
        return convert_1x1_conv_to_mm(x, weight, bias, stride=stride, padding=padding)

    if (
        _use_im2col_mm()
        and ndim == 2
        and not is_ones(kernel_shape)
        and not transposed
        and is_zeros(output_padding)
        and not is_strict_depthwise
    ):
        # The im2col path lowers through mm/addmm/bmm, which already support
        # integer dot inputs via the int32-accumulating GEMM templates.
        return convert_conv2d_to_im2col_mm(
            x,
            weight,
            bias,
            stride=stride,
    ...
    if bias is not None and (device_type != "cpu" or chipyard_backend):
        # Peel off the bias when the backend is faster or only correct without it.
        # Triton chipyard conv templates only take X/W inputs, so bias must be
        # applied as a separate pointwise add.
    ...
    choices = []
    int8_accum_conv = chipyard_backend and _is_integer_conv_dot_input(
        x
    ) and _is_integer_conv_dot_input(weight)
    cast_x_to_int8 = chipyard_backend and _is_integer_conv_dot_input(x)
    cast_w_to_int8 = chipyard_backend and _is_integer_conv_dot_input(weight)

    if torch._inductor.utils._use_conv_autotune_backend("ATEN") and not int8_accum_conv:
        choices = [
    ...
    triton_template_ok = (
        (
            use_triton_template(layout, enable_int32=True)
            if int8_accum_conv
            else use_triton_template(layout, enable_int8=True)
        )
        if chipyard_backend
        else use_triton_template(layout)
    )
    channel_check = (
        V.graph.sizevars.statically_known_equals(in_chan * groups, x.get_size()[1])
        if chipyard_backend
        else V.graph.sizevars.statically_known_equals(in_chan, x.get_size()[1])
    )

    if (
        torch._inductor.utils._use_conv_autotune_backend("TRITON")
        and triton_template_ok
        # templates only support these:
        and is_ones(dilation)
        and not transposed
        and is_zeros(output_padding)
        # there are some odd models where this check fails (e.g. shufflenet_v2_x1_0)
        and channel_check  # type: ignore[arg-type]
    ):
        if (
        ...
        for cfg in conv_configs(
            ...
            **mm_config_kwargs(device_type, _is_large_block_for_cpu),
            ...
            if ndim == 2:
                if chipyard_backend and is_strict_depthwise:
                    depthwise_conv2d_template.maybe_append_choice(
                        ...
                        CAST_X_TO_INT8=cast_x_to_int8,
                        CAST_W_TO_INT8=cast_w_to_int8,
                        OTHER_X=_conv_masked_load_other_value(x),
                        OTHER_W=_conv_masked_load_other_value(weight),
    ...
    if (
        len(choices) == 0
        and chipyard_backend
        and is_strict_depthwise
        and ndim == 2
        and not is_ones(kernel_shape)
        and not transposed
        and is_zeros(output_padding)
