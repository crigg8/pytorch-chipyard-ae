...
def set_driver_to_cpu():
    ...
    try:
        import torch._inductor.config as _config
        cpu_backend = _config.cpu_backend
    except Exception:
        cpu_backend = None
    if backend := triton.backends.backends.get(cpu_backend, None):
        # Avoid forcing default-driver discovery (which can fail when no backend
        # reports is_active=True on CPU-only setups).
        current = getattr(driver, "_active", None)
        if current is None:
            try:
                ...
                current = None
        if (
            isinstance(current, backend.driver)
            or hasattr(current, "_obj")
            and isinstance(current._obj, backend.driver)
        ):
