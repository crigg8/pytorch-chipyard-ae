...
class CachingAutotuner(KernelInterface):
    ...
    def _precompile_config(self, cfg: Config) -> CompileResult[_KernelType]:
        ...
        if (
            self.device_props.type == "cpu"
            and torch._inductor.config.cpu_backend == "triton_chipyard"
        ):
            for key in ("gemmini_tile_i", "gemmini_tile_j", "gemmini_tile_k"):
                if key in compile_meta:
                    options[key] = compile_meta[key]
        compile_kwargs = {
            "target": target,
            "options": options,
        }
    ...
    def bench(self, launcher, *args, with_profiler=False, **kwargs):
        ...
        n_spills = getattr(launcher, "n_spills", None)
        if n_spills is None:
            n_spills = 0
        ...
        if not self.custom_kernel and n_spills > self.inductor_meta.get(
            ...
            log.debug(
                "Skip config %s because of register spilling: %s",
                launcher.config,
                launcher.n_spills,
            )
            return float("inf")
        ...
        if self.device_props.type == "cpu":
            if torch._inductor.config.cpu_backend == "triton_chipyard":
                return benchmarker.benchmark_cpu(kernel_call, warmup=0)
    ...
    def benchmark_all_configs(self, *args, **kwargs):
        with (
            ...
            if log.isEnabledFor(logging.DEBUG):
                ...
                for k, v in timings.items():
                    log.debug(
                        "%s: %f, nreg %d, nspill %s, #shared-mem %s",
                        k.config,
                        v,
                        k.n_regs,
                        k.n_spills,
    ...
    def autotune_to_one_config(self, *args, **kwargs):
        ...
        log.debug(
            "Best config for %s: %s: %f, nreg %d, nspill %s, #shared-mem %s",
            self.fn.__name__,
            launcher.config,
            timings[launcher],
            launcher.n_regs,
    ...
    def coordinate_descent_tuning(self, launcher, *args, **kwargs):
        ...
        def benchmark_one_config(config):
            ...
            log.debug(
                "COORDESC: %s: %f, nreg %d, nspill %s, #shared-mem %d",
                launcher.config,
                out,
                launcher.n_regs,
                launcher.n_spills,
...
class TritonCompileResult(CompileResult[CompiledKernel]):
    ...
    def make_launcher(self) -> LauncherType:
        ...
        binary = self.kernel
        fn = binary.src.fn
        if compile_meta.get("device_type") == "cpu":
            triton_helpers.set_driver_to_cpu()
...
def _reduction_configs(
    *, size_hints: dict[str, int], inductor_meta: dict[str, Any]
    ...
    if inductor_meta.get("disable_reduction_split_configs"):
        return [make_config(1, rnumel, register_intensive=register_intensive)]
