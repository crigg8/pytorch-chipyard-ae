...
from torch._inductor.compile_worker.utils import (
...
class AsyncCompile:
    ...
    def triton(self, kernel_name: str, source_code: str, device_str: str = "cuda"):
        ...
        """
        chipyard_extern_call_snapshot: Optional[str] = None
        if torch._inductor.config.cpu_backend == "triton_chipyard":
            if in_toplevel_process():
                from torch._inductor.codegen.chipyard.extern_calls import (
                    freeze_triton_chipyard_extern_call_registry,
                )

                chipyard_extern_call_snapshot = (
                    freeze_triton_chipyard_extern_call_registry()
                )
            else:
                chipyard_extern_call_snapshot = os.environ.get(
        ...
        if is_parallel:
            ...
            if chipyard_extern_call_snapshot is not None:
                extra_env["TRITON_CHIPYARD_EXTERN_CALL_REGISTRY"] = (
                    chipyard_extern_call_snapshot
                )
            extra_config = {
                "use_static_cuda_launcher": torch._inductor.config.use_static_cuda_launcher,
                "cpu_backend": torch._inductor.config.cpu_backend,
            }
