...
def _is_integer_b2b_dot_input(t: TensorBox) -> bool:
...
{{def_kernel("A", "B", "C")}}
    ...
    for _ in range(num_o_block):
        ...
        acc_ab = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_O), dtype={% if INNER_INT8_DOT %}tl.int32{% else %}tl.float32{% endif %})
        offs_n = tl.arange(0, BLOCK_SIZE_N)
        for __ in range(num_n_block):
            a_mask = (offs_m[:, None] < M) & (offs_n[None, :] < N)
            a_ptrs = A + (offs_m[:, None] * stride_am + offs_n[None, :] * stride_an)
            a = tl.load(a_ptrs, mask=a_mask, other=0.0){% if CAST_A_TO_INT8 %}.to(tl.int8){% else %}.to(tl.float32){% endif %}  # BLOCK_SIZE_M * BLOCK_SIZE_N
            b_mask = (offs_n[:, None] < N) & (offs_o[None, :] < O)
            b_ptrs = B + (offs_n[:, None] * stride_bn + offs_o[None, :] * stride_bo)
            b = tl.load(b_ptrs, mask=b_mask, other=0.0){% if CAST_B_TO_INT8 %}.to(tl.int8){% else %}.to(tl.float32){% endif %}  # BLOCK_SIZE_N * BLOCK_SIZE_O
            acc_ab += tl.dot(a, b, out_dtype={% if INNER_INT8_DOT %}tl.int32{% else %}tl.float32{% endif %})
            offs_n += BLOCK_SIZE_N
        # apply the subgraph
        {{ modification(
            subgraph_number=0,
        ...
        {% if INNER_INT8_DOT %}
        post_subgraph_acc_ab = post_subgraph_acc_ab.to(tl.float32)
        {% endif %}
...
{{def_kernel("A", "B", "C")}}
    ...
    for _ in range(num_n_block):
        ...
        acc_bc = tl.zeros((BLOCK_SIZE_N, BLOCK_SIZE_P), dtype={% if INNER_INT8_DOT %}tl.int32{% else %}tl.float32{% endif %})
        offs_o = tl.arange(0, BLOCK_SIZE_O)
        for __ in range(num_o_block):
            ...
            c_mask = (offs_o[:, None] < O) & (offs_p[None, :] < P)
            c_ptrs = C + (offs_o[:, None] * stride_co + offs_p[None, :] * stride_cp)
            c = tl.load(c_ptrs, mask=c_mask, other=0.0){% if CAST_C_TO_INT8 %}.to(tl.int8){% else %}.to(tl.float32){% endif %}  # BLOCK_SIZE_O * BLOCK_SIZE_P
            acc_bc += tl.dot(b, c, out_dtype={% if INNER_INT8_DOT %}tl.int32{% else %}tl.float32{% endif %})
            offs_o += BLOCK_SIZE_O
        # apply the subgraph
        {{ modification(
            subgraph_number=0,
        ...
        {% if INNER_INT8_DOT %}
        post_subgraph_acc_bc = post_subgraph_acc_bc.to(tl.float32)
        {% endif %}
...
def tuned_b2b_gemm(
    ...
    *,
    ...
    integer_inner_dot_inputs = (
        _is_integer_b2b_dot_input(A) and _is_integer_b2b_dot_input(B)
        if is_left_assoc
        else _is_integer_b2b_dot_input(B) and _is_integer_b2b_dot_input(C)
    )
    inner_mm_dtype = torch.int32 if integer_inner_dot_inputs else A.get_dtype()
    subgraph_buffer = build_subgraph_buffer(
        [create_placeholder("inner_mm", inner_mm_dtype, A.get_device_or_error())],
        subgraph,
    )
    ...
    for config in b2b_gemm_configs:
        if is_left_assoc:
            cur_config = dict(
                config,
                INNER_INT8_DOT=integer_inner_dot_inputs,
                CAST_A_TO_INT8=_is_integer_b2b_dot_input(A),
                CAST_B_TO_INT8=_is_integer_b2b_dot_input(B),
            )
            ...
            cur_config = dict(
                ...
                INNER_INT8_DOT=integer_inner_dot_inputs,
                CAST_B_TO_INT8=_is_integer_b2b_dot_input(B),
                CAST_C_TO_INT8=_is_integer_b2b_dot_input(C),
            )
