...
def _force_triton_chipyard_pool_path(x):
    return x.get_device().type == "cpu" and config.cpu_backend == "triton_chipyard"
...
def max_pool2d_with_indices_backward(
    ...
    *_, pooled_height, pooled_width = grad_output.get_size()
    ...
    if window_size > 25 and not _force_triton_chipyard_pool_path(x):
...
def _avg_poolnd(
    ...
    window_size = functools.reduce(operator.mul, kernel_size)
    force_native_triton_path = _force_triton_chipyard_pool_path(x)
    if window_size > 25 and not force_native_triton_path:
        # Kernel size too big. Results in hard-to-optimize Triton code. Use fallback.
        if dim == 2:
...
def avg_pool3d_backward(
    ...
    *_, pooled_depth, pooled_height, pooled_width = grad_output.get_size()
    ...
    if window_size > 125 and not _force_triton_chipyard_pool_path(x):
