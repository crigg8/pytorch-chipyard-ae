...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile or validate ResNet50 Chipyard artifacts.")
...
def build_model(seed: int) -> torch.nn.Module:
    ...
    model = resnet50(weights=MODEL_WEIGHTS).to(device="cpu", dtype=DTYPE).eval()
...
def run_validate(args: argparse.Namespace) -> None:
    ...
    if not isinstance(inputs, torch.Tensor) or not isinstance(observed, torch.Tensor):
        raise TypeError("ResNet50 artifacts must contain one input tensor and one output tensor")
