...
TASK_NAME = "squeezenet"
MODEL_NAME = "torchvision.squeezenet1_1"
MODEL_WEIGHTS = SqueezeNet1_1_Weights.DEFAULT
DTYPE = torch.float32
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile or validate SqueezeNet Chipyard artifacts.")
...
def build_model(seed: int) -> torch.nn.Module:
    ...
    model = squeezenet1_1(weights=MODEL_WEIGHTS).to(device="cpu", dtype=DTYPE).eval()
...
def run_validate(args: argparse.Namespace) -> None:
    ...
    if not isinstance(inputs, torch.Tensor) or not isinstance(observed, torch.Tensor):
        raise TypeError("SqueezeNet artifacts must contain one input tensor and one output tensor")
